...
<?php 
  ...
  if (!$db->tieneAccesoFile($estructura['Administracion'],'Configuración Contable','nuevo_configuracioncontabilidad.php')){
...
<div class="contemHeaderTop">
    <table cellpadding='0' cellspacing='0' width='100%'>
      <tr class='cabeceraInicialListar'> 
        ...
        <input type="button" name="Guadar" id="Guadar" class="botonNegro" 
         onclick="ejecutarTransaccioncontable()" value="Guardar [F2]" />
     
     </td>
    <td> <input type="hidden" id="transaccion" name="transaccion" value="registrar" />
    <input type="hidden" id="transaccion" name="transaccion" value="registrar" />
    </td>
    <td colspan="3" align='right'><table width="356" border="0">
      <tr>
        <td width="142" colspan="2" align="center"><strong> Configuración de Contabilidad</strong></td>
      </tr>
      <tr>
    ...
    <tr>
      ...
      <div>        
         <ul  class="menujs">
          ...
          <a>Ingreso Dinero</a></li>
          <li id="tabs2" class="listajs" onclick="viewMenu('tabs-2')"><a>Egreso Dinero</a></li>
         </ul>     
  ...
  <tr>
    <td width="18%"><select name="modeloplan" id="modeloplan"  style="width:190px;font-size:12px;visibility:hidden">
    ...
    <td width="20%" align="right" class="letras">Descripción:</td>
    <td width="26%"><input type="text"  id="descripcionS6" onkeyup="insertarCuenta(event,'agregarS6')"/></td>
    <td width="35%"><input type="button" onclick="insertarNewItem('detalleS6','descripcionS6');" id="agregarS6" value="Agregar [Enter]" class="botonNegro"  /></td>
    <td width="1%">&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<div style="position:relative;overflow:auto;height:230px;border:1px solid #E2E2E2;width:70%;margin:0 auto;">
          <table width="100%" border="0" id="tabla" style="margin-top:5px;">
			...
			$contadorCuentas = 0;
			$codigoCuentas = array();
			$sqlcuentas = "select (select pp.cuenta from plandecuenta pp where pp.codigo=( left(ph.codigo,2) ))as 'padre',ph.codigo,ph.cuenta from plandecuenta ph 
     	     where ph.nivel=5 order by ph.codigo;";
            $sql = "select *from tipoconfiguracion where tipo='Ingreso Dinero' order by idtipoconfiguracion";
			$dato = $db->consulta($sql);

			while ($configuracion = mysql_fetch_array($dato)){
			$id = "DS6_".$contadorCuentas;	
			echo "
			<tr>
  ...
  <tr>
    ...
    <td width="26%"><input type="text"  id="descripcionS7" onkeyup="insertarCuenta(event,'agregarS7')"/></td>
    <td width="35%"><input type="button" onclick="insertarNewItem('detalleS7','descripcionS7');" id="agregarS7" value="Agregar [Enter]" class="botonNegro"  /></td>
    <td width="1%">&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<div style="position:relative;overflow:auto;height:230px;border:1px solid #E2E2E2;width:70%;margin:0 auto;">
          <table width="100%" border="0" id="tabla" style="margin-top:5px;">
			...
			$contadorCuentas = 0;
			$codigoCuentas = array();
            $sql = "select *from tipoconfiguracion where tipo='Egreso Dinero' order by idtipoconfiguracion";
			$dato = $db->consulta($sql);

			while ($configuracion = mysql_fetch_array($dato)){
			$id = "DS7_".$contadorCuentas;	
			echo "
			<tr>
              <td width='38' align='center'><img src='css/images/borrar.gif' title='Borrar' onclick='eliminarFila(this)' /></td>
              <td width='38' style='display:none' >$id</td>
