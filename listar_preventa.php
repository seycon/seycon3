...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Pre Venta'
	,'listar_preventa.php','nuevo_preventa.php');
...
<script language="javascript">
	...
	function eliminarTransaccion(){
	  location.href = "anular_registro.php?idpreventa="+$$("idDelete").value+"&tabla=preventa&menu=3";
	}
	...
	var openMensaje = function(codigo){
		...
		$$("modal_contenido").innerHTML = '¿Desea anular esta Pre Venta?';
	...
	function accionPostRegistro() {
	   window.open('preventa/imprimir_preventa.php?idpreventa='+$$("idDelete").value
...
<form name="listado" id="listado" method='get' action='listar_preventa.php#t3' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				   echo "<option value=p.idpreventa>Nº</option>";
	               echo "<option value=c.nombre>Nombre Cliente</option>";
	               echo "<option value=a.nombre>Almacén</option>";
	               echo "<option value=p.modalidad>Modalidad</option>";
     	           echo "<option value=p.moneda>Moneda</option>";
...
<?php
	...
	$condicion = " and ".$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha p.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion = " and p.estado = ".$_GET['estado'];
		} else  {
	...
	$sql = "select p.idpreventa,p.fecha,p.moneda,p.modalidad
	,left(c.nombre,20)as 'cliente',left(a.nombre,20)as 'almacen',
	left(concat(t.nombre,' ',t.apellido) ,20)as 'trabajador' 
	 from preventa p,cliente c,almacen a,usuario u,trabajador t 
	 where p.idcliente=c.idcliente 
	 and a.idalmacen=p.idalmacen 
	 and p.idusuario=u.idusuario 
	 and u.idtrabajador=t.idtrabajador "
	 .$condicion." order by p.idpreventa desc";
...
<?php
	$nombres = array(  "Nº", "Fecha", "Moneda", "Modalidad", "Cliente", "Almacén", "Usuario");
...
if ($fileAcceso['Modificar'] == "Si") { 
	 echo "<td><div align='center'><a href='nuevo_preventa.php?idpreventa=".$id."&sw=1#t3' onclick='return modificar(this)'> 
	...
	if ($fileAcceso['Eliminar'] == "Si") { 
	    echo "<td><div align='center' class='cursorDelete'>
		 <img src='css/images/borrar.gif' title='Anular' alt='borrar' border='0' onclick='openMensaje($id)'/><br />
		 </div></td>";
	} else {
		echo "<td>
		 <div align='center'>
		 <img src='css/images/borrar.gif'  title='Sin Privilegios para Anular' alt='borrar' border='0' />
		 <br /></div></td>";	
	}
}
