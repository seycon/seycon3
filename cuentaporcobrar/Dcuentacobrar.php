...
<?php
	...
	if ($tipo == "numero") {
	    ...
	    echo $db->getNextID("numerocuenta","cuentaporcobrar where idsucursal=$sucursal");	
	}
	...
	{	
	  ...
	  $descripcionLibro = "Cuenta por cobrar Nº $codigo/$tipopersona: $nombrepersona/Sucursal: $datosSucursal[nombrecomercial]";
	...
	{
		$sql = "select max(l.numero)+1 as 'num',l.idsucursal as 'sucursal' from librodiario l where l.idsucursal=$sucursal 
		...
		,'$glosa','$codigo','$tc','$usuario',1,'Cuenta por cobrar');"; 
		$db->consulta($sql);
	...
	if ($tipo == "insertar") {
		...
		$numero = $db->getNextID("numerocuenta","cuentaporcobrar where idsucursal=$_GET[sucursal]");
		
		if ($_GET['moneda'] == "Dolares") {
		    $_GET['importe'] =  round(($_GET['importe'] * $_GET['tipocambio']),4);	
		}
		
		$sql = "INSERT INTO cuentaporcobrar(fecha,numerocuenta,tipodeudor
		,moneda,tipocambio,idsucursal,monto,glosa,fechavencimiento
		 ,iddeudor,cuenta,cuentacaja,idusuario,montoactualcobrado,estado,tipocuenta,documento) 
		 values ('$fecha','$numero','".filtro($_GET['receptor'])."','"
		 .filtro($_GET['moneda'])."','".filtro($_GET['tipocambio'])
		 ."','".filtro($_GET['sucursal'])."','".filtro($_GET['importe'])."','".filtro($_GET['glosa']).  
		 "','$fechaVencimiento','".filtro($_GET['idpersonarecibida'])."','".filtro($_GET['cuenta'])."','"
		 .filtro($_GET['cuentacaja'])."','$_SESSION[id_usuario]',0,1,'".filtro($_GET['tipocuenta'])."','"
		 .filtro($_GET['documento'])."');";
		$db->consulta($sql);	
		$codigo = $db->getMaxCampo('idporcobrar','cuentaporcobrar');
		...
		$dato = $db->arrayConsulta($sql);
		 ...
		 if ($dato['porcobrarhablibrodiario'] == "1") {
			...
			$detalleLibro = getDetalleLibro($db,$_GET['sucursal'],$codigo,$_GET['receptor'],filtro($_GET['nombrepersona']));	
			insertarDetalle($db,$idlibro,$_GET['cuenta'],$detalleLibro,$_GET['importe'],0,'');
			insertarDetalle($db,$idlibro,$_GET['cuentacaja'],$detalleLibro,0,$_GET['importe'],'');
		 }		 
		 
		 echo $codigo."---";
		 $sql = "select ctaxcobdireimprimir from impresion;";  
		 $dato = $db->arrayConsulta($sql);
		 echo $dato['ctaxcobdireimprimir']."---";
	}
	...
	function modificarLibro($sucursal, $moneda, $fecha, $codigo, $tc, $usuario, $db, $glosa)
	{
	  ...
	  where transaccion='Cuenta por cobrar' and idtransaccion=$codigo;";  
	...
	if ($tipo == "modificar") {
		...
		$sql = "update cuentaporcobrar set fecha='$fecha',tipodeudor='".filtro($_GET['receptor'])."',moneda='"
		.filtro($_GET['moneda'])."',tipocambio='".
		filtro($_GET['tipocambio'])."',idsucursal='".filtro($_GET['sucursal'])
		."',monto='".filtro($_GET['importe'])."',glosa='"
		.filtro($_GET['glosa'])."',fechavencimiento='$fechaVencimiento',iddeudor='"
		.filtro($_GET['idpersonarecibida'])."',tipocuenta='".filtro($_GET['tipocuenta'])
		."',documento='".filtro($_GET['documento'])."',
		 cuenta='".filtro($_GET['cuenta'])."',cuentacaja='".filtro($_GET['cuentacaja'])
		 ."',idusuario='$_SESSION[id_usuario]' where idporcobrar='$_GET[idporcobrar]';";
		...
		$dato = $db->arrayConsulta($sql);
		if ($dato['porcobrarhablibrodiario'] == "1") {	
		   $idlibro = modificarLibro($_GET['sucursal'],'Bolivianos',$fecha,$_GET['idporcobrar']
		   ,$_GET['tipocambio'],$_SESSION['id_usuario'],$db,filtro($_GET['glosa']));
		   $detalleLibro = getDetalleLibro($db,$_GET['sucursal'],$_GET['idporcobrar'],filtro($_GET['receptor'])
	...
	if ($tipo == "reporte") {
		...
		WHERE LEFT( dl.idcuenta, 7) =  '1.1.02.'
		...
		$sql = "SELECT p.codigo, p.nivel, p.cuenta, IF( p.moneda =  'Bolivianos',  'Bs',  '$us' ) AS  'moneda'
		, SUM( dl.debe ) AS  'debe', SUM( dl.haber ) AS  'haber', (SUM(dl.debe) - SUM(dl.haber)
		...
		$monedaSub = "";
				  ...
				  continue; 
					...
					if ($padre != "" && $padre != $balance['padre']) {
						...
						echo $subTabla;
						 ...
						 <div style='width:".abs($por)."%;' class='barra'></div></td><td></td></tr>";
						...
						for($i = 0; $i < count($subPorcentajes); $i++) {
						   ...
						   <td width='85%'  height='22px'><div style='width:".abs($auxPorcentaje)."%;'
							class='barraSecundaria'></div></td><td></td></tr>";
						}
		  ...
		  if ($padre != "") {
			  ...
			  echo $subTabla;
			   ...
			   $totalSubPorcentaje = abs($por);
			   $auxPorcentaje = (abs($por)>100) ? 100 : abs($por);
			   $porcentajes = $porcentajes."<tr><td width='15%' class='fondoCeldaBarra'>$por%</td><td width='85%' height='22px'>
			   <div style='width:".abs($por)."%;' class='barra'></div></td><td></td></tr>";
