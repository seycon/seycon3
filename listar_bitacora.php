...
if (!$db->tieneAccesoFile($estructura['Administracion'],'Bitácora','listar_bitacora.php')){
...
function anular(){
if (confirm("Desea anular este elemento de la tabla bitacora?")==false){
return false;
}else{
return true;
}
...
<div class="cabeceraFormulario">
  <div class="menuTituloFormulario"> Administración > Bitácora </div>
  <div class="menuFormulario"> 
   <?php
     $estructura = $_SESSION['estructura'];
     $menus = $estructura['Administracion'];
     $privilegios = $db->getOpciones($menus, "Bitácora"); 
     $option = "";
...
<form name="listado" id="listado" method='get' action='listar_bitacora.php#t19' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idbitacora>Nº</option>";
				echo "<option value=tipodemodificacion>Transaccion</option>";
				echo "<option value=tabla>Registro</option>";
				echo "<option value=t.nombre>Nombre Trabajador</option>";
			  ?>
            </select>
   </div> 
...
location.href='listar_bitacora.php?abuscar='+document.getElementById('input').value+'&campo='+document.getElementById('filtro').value+'&estado=1';
else
location.href='listar_bitacora.php?abuscar='+document.getElementById('input').value+'&campo='+document.getElementById('filtro').value+'&estado=0';
}
</script>
...
$cFecha = ($_GET['fecha'] == "") ? "" : " date_format(b.fecha,'%d/%m/%Y') ='".$_GET['fecha']."' and ";
$condicion = " where b.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and  $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%'";
}
else 
if (isset($_GET['estado'])) 
$condicion= " where b.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador  ";
else 
$condicion= " where b.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador ";


$sql = "SELECT idbitacora,date_format(b.fecha,'%d/%m/%Y %H:%i:%s'),left(tipodemodificacion,30),tabla,left(concat(t.nombre,' ',t.apellido),25),b.observaciones from bitacora b,usuario u,trabajador t ".$condicion." order by idbitacora desc";
...
<?php if ($numeroPagina > 0) { ?>
   <a href="<?php printf("%s?numPage=%d%s#t19", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t19", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
<?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t19", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t19", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
	$nombres = array(  "Nº", "Fecha/Hora", "Transacción", "Registro" ,"Usuario", "Observaciones", "" );
