...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Cargo','listar_cargo.php','nuevo_cargo.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idcargo="+$$("idDelete").value+"&tabla=cargo&menu=5";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Cargo?';
...
<form name="listado" id="listado" method='get' action='listar_cargo.php#t5' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idcargo>Nº</option>";
...
<?php
	...
	$sql = 'SELECT idcargo,left(c.cargo,25),left(c.descripcion,80) from cargo c '.$condicion.'  order by idcargo desc';
...
<?php
$nombres = array(  "Nº", "Cargo", "Descripción");
...
if ($fileAcceso['Modificar'] == "Si") { 
	 echo "<td><div align='center'><a href='nuevo_cargo.php?idcargo=".$id."&sw=1#t5' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
