...
$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Solicitud de Producto','listar_solicitud.php','nuevo_solicitud.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idsolicitud="+$$("idDelete").value+"&tabla=solicitud&menu=2";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Solicitud?';
...
function accionPostRegistro(){
   window.open('solicitud_pedidos/imprimir_SolitudProductos.php?idsolicitud='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<div class="cabeceraFormulario">
    <div class="menuTituloFormulario"> Inventario > Solicitud de Producto </div>
...
<form name="listado" id="listado" method='get' action='listar_solicitud.php#t2' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				  echo "<option value=s.idsolicitud>Nº</option>";
				  echo "<option value=a.nombre>Sucursal</option>";	
  				  echo "<option value=p.nombre>Proveedor</option>";	  
				  echo "<option value=s.monto>Monto</option>";	
...
<tr>
<td width='296' ><?php if (isset($_GET['estado']) && ($_GET['estado'] == 0))  echo '';?></td>
<td width='95' style='font-family: Verdana, Geneva, sans-serif;'>&nbsp;</td>
<td width='64' style='font-family: Verdana, Geneva, sans-serif;'><span style="width:10px;">
 
    <div align="right"> Activos </div>
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		$cFecha = ($_GET['fecha'] == "") ? "" : " s.fecha ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";
		$condicion = " and  $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and s.estado=".$_GET['estado'];
	} else {
	    if (isset($_GET['estado'])) {
	        $condicion= " and s.estado = ".$_GET['estado'];
		} else {
	        $condicion= ' and s.estado =1 ';
		}
	}

	$sql = "SELECT idsolicitud,s.fecha,left(sl.nombrecomercial,25)as nombre,left(p.nombre,25)
	        ,round(monto,2)as monto,left(concat(t.nombre,' ',t.apellido),25)as usuario
	        from 
			solicitud s, almacen a,usuario u,trabajador t,proveedor p,sucursal sl 
			WHERE u.idusuario=s.idusuario  and a.sucursal=sl.idsucursal 
			and u.idtrabajador=t.idtrabajador and s.idalmacen = a.idalmacen and 
			s.idproveedor=p.idproveedor ".$condicion."  order by  
			idsolicitud desc";
...
$nombres = array(  "Id", "Fecha", "Sucursal", "Proveedor", "Monto", "Usuario" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)){	
   ...
   if ($fileAcceso['Modificar'] == "Si") { 
	 echo "<td><div align='center'><a href='nuevo_solicitud.php?idsolicitud=".$id."&sw=1#t2' onclick='return modificar(this)'><img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";	
	} else {
