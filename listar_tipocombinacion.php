...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Tipo Combinación'
	,'listar_tipocombinacion.php','nuevo_tipocombinacion.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idtipocombinacion="+$$("idDelete").value+"&tabla=tipocombinacion&menu=10";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Tipo de Combinación?';
...
<div class="cabeceraFormulario">
  <div class="menuTituloFormulario"> Inventario > Tipo Combinación </div>
...
<form name="listado" id="listado" method='get' action='listar_tipocombinacion.php#t10' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idtipocombinacion>Nº</option>";
...
<?php
	...
	if ($_GET['abuscar'] != '') {
		$condicion= " where ".$_GET['campo']." like '".$_GET['abuscar']."%' and t.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= " where t.estado = ".$_GET['estado']." ";
		} else { 
			$condicion= ' where t.estado =1 ';
		}
	}

	$sql = " SELECT idtipocombinacion,t.nombre,left(t.descripcion,45)as 'descripcion',
	left(concat(tb.nombre,' ',tb.apellido),25)as 'trabajador' from tipocombinacion t,usuario u,trabajador tb "
	.$condicion." and t.idusuario=u.idusuario and u.idtrabajador=tb.idtrabajador  order by idtipocombinacion desc";
...
$nombres = array(  "idtipocombinacion", "nombre", "descripcion", "responsable");
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_tipocombinacion.php?idtipocombinacion=".$id."&sw=1#t10' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
