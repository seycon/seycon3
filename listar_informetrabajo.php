...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Informe de Trabajo','listar_informetrabajo.php','nuevo_informetrabajo.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idinformetrabajo="+$$("idDelete").value+"&tabla=informetrabajo&menu=20";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Informe Trabajo?';
...
function accionPostRegistro(){
   window.open('informetrabajo/imprimir_informeTrabajo.php?idinformetrabajo='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');
   cerrarPagina();	
}
...
<form name="listado" id="listado" method='get' action='listar_informetrabajo.php#t20' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idinformetrabajo>Nº</option>";
				echo "<option value=estadocobranza>Estado Cobranza</option>";
...
if (isset($_GET['abuscar'])){
  ...
  $condicion = " where u.idusuario=i.idusuario and u.idtrabajador=t.idtrabajador and ".$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha i.estado=".$_GET['estado'];
}
else 
if (isset($_GET['estado'])) 
$condicion = " where u.idusuario=i.idusuario and u.idtrabajador=t.idtrabajador and i.estado = ".$_GET['estado'];
else 
$condicion = ' where u.idusuario=i.idusuario and u.idtrabajador=t.idtrabajador and i.estado = 1';


$sql = "select idinformetrabajo,estadocobranza,date_format(fecha,'%d/%m/%Y'),nrofactura,left(concat(t.nombre,' ',t.apellido),25) from informetrabajo i,usuario u,trabajador t ".$condicion." order by idinformetrabajo desc";
...
<? if (isset($_GET['estado']) && ($_GET['estado'] == 1))  echo ' checked ';
if (!isset($_GET['estado']))
 echo ' checked';
...
<?php if ($numeroPagina > 0) { ?>
   <a href="<?php printf("%s?numPage=%d%s#t20", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t20", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
<?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t20", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t20", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
$nombres = array(  "Nº", "Estado Cobranza", "Fecha", "Nº Factura", "Responsable" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_informetrabajo.php?idinformetrabajo=".$id."#t20' > <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
