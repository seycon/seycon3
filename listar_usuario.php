...
$fileAcceso = $db->privilegiosFile($estructura['Administracion'],'Usuarios de Sistema','listar_usuario.php','nuevo_usuario.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idusuario="+$$("idDelete").value+"&tabla=usuario&menu=2";
}
...
<form name="listado" id="listado" method='get' action='listar_usuario.php#t2' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=u.idusuario>Nº</option>";
				echo "<option value=nombre>Nombre Trabajador</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		$cFecha = ($_GET['fecha'] == "") ? "" : " fechacreacion ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";		
		$condicion= " where t.idtrabajador = u.idtrabajador and s.idsucursal=t.idsucursal and ".$_GET['campo']
			." like '".$_GET['abuscar']."%' and $cFecha u.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= " where t.idtrabajador = u.idtrabajador and s.idsucursal=t.idsucursal and u.estado = ".$_GET['estado'];
		} else { 
			$condicion= ' where t.idtrabajador = u.idtrabajador and s.idsucursal=t.idsucursal and u.estado =1';
		}
	}

	$sql = "SELECT u.idusuario, concat(t.nombre,' ',t.apellido)AS  nombre, login,left(s.nombrecomercial,25) , 
	fechacreacion FROM usuario u,trabajador t,sucursal s  ".$condicion." ORDER BY idusuario desc";
...
<?php
$nombres = array( "Nº","Nombres", "Login", "Sucursal" ,"fecha Creación");
...
echo "<td><div align='center'><a href='nuevo_usuario.php?idusuario=".$id."&sw=1#t2' > <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
	...
	for( $i = 0 ; $i <=4 ; $i++ ) {
		  ...
		  if ($i==4){
		  echo "<td class='tdpar' align='center'>".$db->GetFormatofecha($fila[$i],"-")."</td>";
		  }		
		  else{
		   if ($i == 0 || $i == 3 || $i == 2)
