...
function get_datos($mysql, $mes, $anio, $tipo)
{
    if ($tipo=="*") {
        $consulta="select *,sum(monto) as 'total_mes' from notaventa
                where MONTH(fecha)=$mes and YEAR(fecha)=$anio and estado=1
                group by MONTH(fecha)";
    } else {
        $consulta="select *,sum(monto) as 'total_mes' from notaventa
                where MONTH(fecha)=$mes and YEAR(fecha)=$anio and tiponota='$tipo' and estado=1
                group by MONTH(fecha)";
    }
...
<body>  
    ...
    $total_mes= array();    
    for ($i = 1; $i <= 12; $i++) {
        $res= get_datos($mysql, $i, $_GET['anio'],$_GET['tipo']);        
