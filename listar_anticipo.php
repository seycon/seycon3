...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Anticipo','listar_anticipo.php','nuevo_anticipo.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idanticipo="+$$("idDelete").value
  +"&tabla=anticipo&menu=1&libro=Anticipo Sueldo&tipolibro=egreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Anticipo?';
...
<form name="listado" id="listado" method='get' action='listar_anticipo.php#t2' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=a.idanticipo>Nº</option>";
				echo "<option value=t.nombre>Trabajador</option>";
				echo "<option value=a.documento>Doc.</option>";
				echo "<option value=s.nombrecomercial>Sucursal</option>";	
				echo "<option value=a.anticipo>Anticipo</option>";
				echo "<option value=pc.cuenta>Cuenta</option>";		 
...
if (isset($_GET['abuscar'])){
 $cFecha = ($_GET['fecha'] == "") ? "" : " a.fecha ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";
 $condicion= "  a.idsucursal=s.idsucursal and a.idtrabajador=t.idtrabajador and $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and a.estado =".$_GET['estado'];
}
else 
if (isset($_GET['estado'])) 
$condicion= "  a.idsucursal=s.idsucursal and a.idtrabajador=t.idtrabajador and a.estado = ".$_GET['estado'];
else 
$condicion= "  a.idtrabajador=t.idtrabajador and a.idsucursal=s.idsucursal and a.estado = 1";



$sql = "SELECT idanticipo,a.numero,date_format(a.fecha,'%d/%m/%Y')
,left(concat(t.nombre,' ',t.apellido),40),a.documento,left(s.nombrecomercial,20),a.anticipo,left(pc.cuenta,25),left(concat(tu.nombre,' ',tu.apellido),40)
 from anticipo a,trabajador t,trabajador tu,sucursal s,usuario u,plandecuenta pc where
 a.idusuario=u.idusuario and a.egreso=pc.codigo and u.idtrabajador=tu.idtrabajador and ".$condicion." order by idanticipo desc";
...
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t2", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
$nombres = array(  "Nº", "Fecha", "Trabajador", "Doc.", "Sucursal", "Anticipo", "Caja/Banco", "Usuario");
...
echo "<td><div align='center'><a href='nuevo_anticipo.php?idanticipo=".$id."&sw=1#t2' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
...
if ($fileAcceso['Eliminar'] == "Si"){ 
	...
	echo "<td><div align='center'><a href='reportes/recursos/imprimir_anticipo.php?idanticipo=".$id."' target='_blank' onclick='return imprimir(this)'><img src='css/images/imprimir.gif' title='Imprimir' alt='imprimir' border='0' /><br /></a></div></td>";
		for( $i = 1; $i <= 8; $i++ ){
		if($fila[$i] == "")
		{
		...
		else
		{
          switch($i){
			...
			break; 
			case 6:
			  echo "<td align='center'>".number_format($fila[$i],2)."</td>";
			break;
			default:
			echo "<td align='center'>".$fila[$i]."</td>";
