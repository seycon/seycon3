...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Liquidación','listar_finiquitos.php','nuevo_finiquito.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idfiniquitos="+$$("idDelete").value+"&tabla=finiquitos&menu=4";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Finiquito?';
...
<form name="listado" id="listado" method='get' action='listar_finiquitos.php#t11' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idfiniquitos>Nº</option>";
				echo "<option value=motivo>Motivo</option>";
				echo "<option value=t.nombre>Trabajador</option>";	
				echo "<option value=t.carnetidentidad>C.I</option>";			  
			  ?>
            </select>
   </div> 
...
<?php
	...
	if (isset($_GET['abuscar'])) {
	    ...
	    $condicion = " where f.idtrabajador=t.idtrabajador and $cFecha ".$_GET['campo']." like '"
		    .$_GET['abuscar']."%' and f.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion = " where f.idtrabajador=t.idtrabajador and f.estado = ".$_GET['estado'];
		} else { 
			$condicion = ' where f.idtrabajador=t.idtrabajador and f.estado = 1';
		}
	}

	$sql = "SELECT idfiniquitos,motivo,date_format(fecha,'%d/%m/%Y'),left(concat(t.nombre,'" 
	    ." ',t.apellido),25),t.carnetidentidad,t.idtrabajador from finiquitos f,trabajador t "
	    .$condicion." order by idfiniquitos desc";
...
<?php if ($numeroPagina > 0) { ?>
  <a href="<?php printf("%s?numPage=%d%s#t11", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t11", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
 <?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t11", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
        <a href="<?php printf("%s?numPage=%d%s#t11", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
$nombres = array("Iddepartamento", "Motivo", "Fecha", "Trabajador","C.I");
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)){	
	...
	echo "<td><div align='center'><a href='nuevo_finiquito.php?idfiniquitos=".$id."&sw=1#t11' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
...
if ($fileAcceso['Eliminar'] == "Si"){ 
	...
	echo "<td><div align='center'><a href='finiquitos/imprimir_finiquitos.php?trabajador=".$fila[5]."' target='_blank' onclick='return imprimir(this)'><img src='css/images/imprimir.gif' title='Imprimir' alt='imprimir' border='0' /><br /></a></div></td>";
