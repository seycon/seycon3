...
<?php
	...
	,'listar_servicio.php','nuevo_servicio.php');
...
<script language="javascript">
  function eliminarTransaccion(){
	location.href = "anular_registro.php?idservicio="+$$("idDelete").value+"&tabla=servicio&menu=7";
  }
  ...
  var openMensaje = function(codigo){
	  ...
	  $$("modal_contenido").innerHTML = '¿Desea anular este Servicio?';
...
<form name="listado" id="listado" method='get' action='listar_servicio.php#t7' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idservicio>Nº</option>";
				echo "<option value=s.nombre>Nombre</option>";
				echo "<option value=s.descripcion>Descripción</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		$condicion= " where s.idusuario=u.idusuario and 
		    t.idtrabajador=u.idtrabajador and ".$_GET['campo']." like '".$_GET['abuscar']
		    ."%' and s.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= " where s.idusuario=u.idusuario and t.idtrabajador=u.idtrabajador
			 and s.estado= ".$_GET['estado'];
		} else { 
			$condicion= ' where s.idusuario=u.idusuario and
			 t.idtrabajador=u.idtrabajador and s.estado =1';
		}
	}

	$sql = "select idservicio,s.nombre
	,left(s.descripcion,25),left(requerimiento,25)
	,left(concat(t.nombre,' ',t.apellido),30) as 'trabajador'	
	 from servicio s,trabajador t,usuario u ".$condicion." order by idservicio desc";
...
<?php
$nombres = array(  "Nº", "Nombre", "Descripción", "Requerimiento", "Usuario" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_servicio.php?idservicio=".$id."&sw=1#t7' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
