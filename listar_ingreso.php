...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Contabilidad'],'Ingreso de Dinero','listar_ingreso.php','nuevo_ingreso.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idingreso="+$$("idDelete").value+"&tabla=ingreso&menu=1&libro=Ingreso dinero&tipolibro=ingreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Ingreso?';
...
function accionPostRegistro(){
   window.open('ingresos/imprimir_ingreso.php?idingreso='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');
   cerrarPagina();	
}
...
<form name="listado" id="listado" method='get' action='listar_ingreso.php#t1' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idingreso>Nº</option>";
				echo "<option value=s.nombrecomercial>Sucursal</option>";
				echo "<option value=recibo>Doc.</option>";
				echo "<option value=cheque>Cheque</option>";
				echo "<option value=nombrepersona>Beneficiario</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])) {
	 ...
	 $condicion = " where i.idsucursal=s.idsucursal and i.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and "
	 .$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha i.estado=".$_GET['estado'];
	} else {  
	 if (isset($_GET['estado'])) { 
	 $condicion = " where i.idsucursal=s.idsucursal and i.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and i.estado = "
	 .$_GET['estado'];
	 } else {  
	 $condicion = ' where i.idsucursal=s.idsucursal and i.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and i.estado = 1';
	 }
	}

$sql = "SELECT idingreso,fecha,left(s.nombrecomercial,25),recibo,cheque,left(nombrepersona,11)
,round(ingresoBolivianos,2),round(ingresoDolares,2),left(concat(t.nombre,' ',t.apellido),25)
,tipocambio from ingreso i,
usuario u,trabajador t,sucursal s ".$condicion." order by idingreso desc";
...
<?php if ($numeroPagina > 0) { ?>
   <a href="<?php printf("%s?numPage=%d%s#t1", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t1", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
<?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t1", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>">
<img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t1", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>">
...
<?php
$nombres = array(  "Nº", "fecha", "Sucursal", "Doc.", "Cheque", "Beneficiario","Bs.", "us.", "Usuario" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
    ...
    if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_ingreso.php?idingreso=".$id."&sw=1#t1' onclick='return modificar(this)'>
	  <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	} else {
	 echo "<td><div align='center'><img src='css/images/edit.gif' title='Sin Privilegios para Modificar'
	  alt='editar' border='0' /></div></td>";	
	}
	 ...
	 title='Imprimir' alt='imprimir' border='0' onclick='openVentanaImprimir($id)'/><br /></div></td>";
		...
		echo "<td class='tdpar'>&nbsp;</td>";
          ...
          switch($i){
			...
			break;
			case 7:
			  echo "<td class='centro'>".convertir(round(($fila[$i]/$fila[9]),2))."</td>";
			break;
			default:
