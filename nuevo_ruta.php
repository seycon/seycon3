...
<?php 
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Ruta de Ventas','nuevo_ruta.php','listar_ruta.php');
	...
	if($_POST['transaccion'] == "insertar"){
	  $sql = "INSERT INTO ruta(nombre,descripcion,estado) VALUES 
	  ('".filtro($_POST['nombre'])."','".filtro($_POST['descripcion'])."',1);";
	}
	
	if($_POST['transaccion'] == "modificar"){
	   $sql = "UPDATE ruta SET nombre='".filtro($_POST['nombre'])
	   ."', descripcion='".filtro($_POST['descripcion'])."' where idruta=$_POST[idruta];";
	}
	
	$db->consulta($sql);
	header("Location: nuevo_ruta.php#t9");
	}
...
<script>
  ...
  document.onkeydown = function(e){
   ...
   if (tecla == 115){ //F4
    if ($$("cancelar") != null)
	 location.href = 'listar_ruta.php#t9';
   }
...
<table style="width:75%;top:38px;left:0px; margin: 0 auto;position:relative;"  >
 ...
 <form id='formValidado' name='formValidado' method='post' action='nuevo_ruta.php' enctype='multipart/form-data'>
          ...
          <?php 
              if ($fileAcceso['File'] == "Si"){
               echo '&nbsp;&nbsp;<input name="cancelar" type="button" class="botonNegro" id="cancelar"
                value="Cancelar [F4]" onClick="location.href=&#039listar_ruta.php#t9&#039"/>';	
              }
          ?> 
       </td>
      <td><input type="hidden"  id="transaccion" name="transaccion" value="<?php echo $transaccion;?>" />
      <input type="hidden" id="idruta" name="idruta" value="<?php echo $datoTransaccion['idruta'];?>" /></td>
...
<tr>
  ...
  <table width="90%" border="0" align="center" class="bordeContenido">
    ...
    <tr>
      ...
      <td><input type='text' id="nombre" name="nombre"  class="required"
       style="width:80%" value="<?php echo $datoTransaccion['nombre'];?>" /></td>
      <td align="right"></td>
      <td></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <textarea name="descripcion" id="descripcion" cols="80" rows="3"><?php echo $datoTransaccion['descripcion'];?></textarea>
      </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
