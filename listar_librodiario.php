...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Contabilidad'],'Libro Diario','listar_librodiario.php','nuevo_librodiario.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idlibrodiario="+$$("idDelete").value+"&tabla=librodiario&menu=3";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Libro Diario?';
...
function accionPostRegistro(){
   window.open('librodiario/imprimir_libro.php?idlibrodiario='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_librodiario.php#t3' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=l.numero>Nº</option>";
				echo "<option value=l.moneda>Moneda</option>";	
				echo "<option value=l.transaccion>Transacción</option>";
				echo "<option value=tipotransaccion>Tipo de Transacción</option>";
...
if (isset($_GET['abuscar'])){
 ...
 $condicion = "where l.idsucursal=s.idsucursal and l.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and ".$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha l.estado=".$_GET['estado'];
}
else 
if (isset($_GET['estado'])) 
$condicion = " where l.idsucursal=s.idsucursal and l.idusuario=u.idusuario 
and u.idtrabajador=t.idtrabajador and l.estado = ".$_GET['estado'];
else  
$condicion = ' where l.idsucursal=s.idsucursal and l.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and l.estado = 1';


$sql = "select idlibrodiario,l.numero,l.moneda,left(l.transaccion,25)as 'transaccion',tipotransaccion,fecha
,left(s.nombrecomercial,25),left(concat(t.nombre,' ',t.apellido),25)
 ,l.idtransaccion from librodiario l,sucursal s,usuario u,trabajador t ".$condicion." order by idlibrodiario desc";
...
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t3", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>">
...
<?php
	$nombres = array(  "Nº", "Moneda", "Transacción", "tipo Transacción","Fecha","Sucursal", "Usuario" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
    ...
    if ($fileAcceso['Modificar'] == "Si" && $fila[8] == "") {   
	 echo "<td><div align='center'><a href='nuevo_librodiario.php?idlibrodiario=".$id."&sw=1#t3'
	...
	for( $i = 1 ; $i <= 7 ; $i++ ) {
		...
		else
		{
          switch($i){
			case 4:
			  echo "<td class='centro'>".ucfirst($fila[$i])."</td>";
			break;  
			case 5:
