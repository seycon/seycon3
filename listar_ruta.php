...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Ruta de Ventas','listar_ruta.php','nuevo_ruta.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idruta="+$$("idDelete").value+"&tabla=ruta&menu=9";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Ruta?';
...
<div class="cabeceraFormulario">
  <div class="menuTituloFormulario"> Ventas > Ruta de Ventas </div>
  <div class="menuFormulario"> 
   <?php
     $estructura = $_SESSION['estructura'];
     $menus = $estructura['Ventas'];
     $privilegios = $db->getOpciones($menus, "Ruta de Ventas"); 
     $option = "";
...
<form name="listado" id="listado" method='post' action='listar_ruta.php#t9' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idruta>Nº</option>";
...
if ($_POST['abuscar']!=''){
 $condicion = " where ".$_POST['campo']." like '".$_POST['abuscar']."%' and estado=".$_POST['estado'];
}
else 
if (isset($_POST['estado'])) 
...
$sql = 'SELECT idruta,nombre,left(descripcion,40) from ruta '.$condicion.' order by idruta desc';
mysql_query("SET NAMES 'utf8'");
$res = consulta($sql);
$n = mysql_num_rows($res);

$nombres = array(  "Nº", "Nombre", "Descripción");
...
if (isset($_POST['estado']) && ($_POST['estado'] == 0)){	
	...
	echo "<td><div align='center'><a href='nuevo_ruta.php?idruta=".$id."&sw=1#t9' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	} else {
