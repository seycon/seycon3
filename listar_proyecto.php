...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Proyecto','listar_proyecto.php','nuevo_proyecto.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idproyecto="+$$("idDelete").value+"&tabla=proyecto&menu=6";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Proyecto?';
...
function accionPostRegistro(){
   window.open('proyecto/imprimir_proyecto.php?idproyecto='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_proyecto.php#tt6' enctype='multipart/form-data'>     
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idproyecto>Nº</option>";
				echo "<option value=titulo>Titulo</option>";
				echo "<option value=t.nombre>Responsable</option>";		  
			  ?>
            </select>
   </div> 
...
location.href='listar_proyecto.php?abuscar='+document.getElementById('input').value+'&campo='+document.getElementById('filtro').value+'&estado=1#t6';
else
location.href='listar_proyecto.php?abuscar='+document.getElementById('input').value+'&campo='+document.getElementById('filtro').value+'&estado=0#t6';
}
</script>
...
if (isset($_GET['abuscar'])){
    $condicion = " where p.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and ".$_GET['campo']." like '"
	.$_GET['abuscar']."%' and p.estado=".$_GET['estado'];
}
else{ 
    if (isset($_GET['estado'])) { 
        $condicion= " where p.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and p.estado = ".$_GET['estado'];
	} else {
       $condicion= ' where p.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and p.estado = 1';
	}
}

$sql = "SELECT idproyecto,left(titulo,40),date_format(p.fechainicio,'%d/%m/%Y'),date_format(p.fechafinalizacion,'%d/%m/%Y'),round(presupuesto,2),left(concat(t.nombre,' ',t.apellido),25) from proyecto p,usuario u,trabajador t ".$condicion." order by idproyecto desc";
...
<?php
$nombres = array(  "Nº", "Titulo", "Fecha Inicio", "Fecha Finalización", "Presupuesto", "Responsable" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)){	
   ...
   if ($fileAcceso['Modificar'] == "Si"){ 
	     echo "<td><div align='center'><a href='nuevo_proyecto.php?idproyecto=".$id."&sw=1#t6' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
