...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Actas de Reunión','listar_acta.php','nuevo_acta.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idacta="+$$("idDelete").value+"&tabla=acta&menu=8";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Acta?';
...
<form name="listado" id="listado" method='get' action='listar_acta.php#t8' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
					echo "<option value=idacta>Nº</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		...
		$condicion = " where a.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and $cFecha "
			.$_GET['campo']." like '".$_GET['abuscar']."%' and a.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion = " where a.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and a.estado = ".$_GET['estado'];
		} else {
			$condicion = " where a.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and a.estado = 1";
		}
	}

	$sql = "SELECT idacta,horainicio,horacierre,date_format(fecha,'%d/%m/%Y'),titulo,"
	    ."left(concat(t.nombre,' ',t.apellido),25) from acta a,usuario u,trabajador t ".$condicion." order by idacta desc";
...
<?php
$nombres = array(  "Id", "Hora Inicio", "Hora Cierre", "Fecha", "titulo", "Responsable" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_acta.php?idacta=".$id."&sw=1#t8' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
