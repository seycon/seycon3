...
$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Almacén','listar_almacen.php','nuevo_almacen.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idalmacen="+$$("idDelete").value+"&tabla=almacen&menu=1";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Almacen?';
...
<form name="listado" id="listado" method='get' action='listar_almacen.php#t1' enctype='multipart/form-data'>
   ...
   <div class="listadoCombo1">
      <div class="borde1Listar"> </div>
        <select id="campo" name="campo" class="borde2Combo1">
          <?php
              echo "<option value=a.idalmacen>Nº</option>";
...
<?php
	...
	if ($_GET['abuscar'] != '') {
		$condicion= " where ".$_GET['campo']." like '".$_GET['abuscar']."%' and a.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= " where a.estado = ".$_GET['estado']." ";
		} else { 
			$condicion= ' where a.estado =1 ';
		}
	}

	$sql = " SELECT a.idalmacen, LEFT( a.nombre, 18 ) AS nombre, LEFT( a.ubicacion, 18 ) AS ubicacion
	, LEFT( s.nombrecomercial, 18 ) AS  nombrecomercial
	, LEFT( observaciones, 18 ) AS observaciones
	, LEFT( CONCAT( t.nombre,  ' ', t.apellido ) , 25 ) AS responsable
	  FROM almacen a
      LEFT JOIN sucursal s ON a.sucursal = s.idsucursal 
	  LEFT JOIN usuario u ON u.idusuario = a.idusuario 
	  LEFT JOIN trabajador t ON t.idtrabajador = u.idtrabajador  
     ".$condicion."  order by a.idalmacen desc";
...
$nombres = array(  "idalmacen", "nombre", "Ubicación", "sucursal", "observaciones", "Usuario" );
...
$id = $fila[0];
 if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {
   ...
   if ($fileAcceso['Modificar'] == "Si") { 
	 echo "<td><div align='center'><a href='nuevo_almacen.php?idalmacen=".$id."&sw=1#t1' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	} else {
	...
	if ($fileAcceso['Eliminar'] == "Si") { 
	 ...
	 echo "<td><div align='center'><img src='css/images/borrar.gif'  
	 title='Sin Privilegios para Anular' alt='borrar' border='0' /><br /></div></td>";	
	}
