...
<?php
	 ...
	 $estructura = $_SESSION['estructura'];
	  $fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Venta de Producto','nuevo_notaventa.php','listar_notaventa.php');
	 ...
	 $sql = "select left(titulonotadeventa,30)as 'titulonotadeventa' from impresion where idimpresion = 1";
	 $tituloPrincipal = $db->getCampo('titulonotadeventa', $sql);	 
...
<link rel="stylesheet" type="text/css" href="factura/style.css" />
...
<script src="factura/NVenta.js"></script>
    ...
    <div class="modal">
          ...
          <table align="center" width="740">
            ...
            <tr>
              ...
              <select id="idLotes" name="idLotes" style="border:1px solid #CCC;width:80%">     
              </select>
              </th>
              <th width="15%">&nbsp;</th>
              <th align="left">&nbsp;</th>
              <th align="left" >Producto en Almacen</th>
            </tr>
            <tr>
              <th></th>
              <th>Cantidad</th>
              ...
              <th width="200" rowspan="5" align="left">
            ...
            <tr>
              ...
              <th><input id="ppreciounitario" name="ppreciounitario"  type="text"  size="10"  
              onkeyup="calcularUnidadMedida(this.value,'precioU');"/></th>
              <th width="12%"><input id="ptotal" name="ptotal"  type="text"  size="12" onkeypress="" disabled="disabled"/></th>
            </tr>
            <tr>
              ...
              type="text"  size="10"  onkeyup="calcularUnidadMedida(this.value,'precioUA');" disabled="disabled"/></th>
              <th height="10">&nbsp;</th>
            </tr>
            <tr>
...
<div class="menuTituloFormulario"> Ventas > Ventas de Productos </div>
<div class="menuFormulario"> 
 <?php
   $estructura = $_SESSION['estructura'];
   $menus = $estructura['Ventas'];
   $privilegios = $db->getOpciones($menus, "Venta de Producto"); 
   $option = "";
...
<table cellpadding='0' cellspacing='0' width='99%' align="center" class="contemHeaderTop">
 ...
 <div id="datos_factura" class="datos_cliente1">
  ...
  <tr>
    <td></td>
    <td colspan="3"><div id="mensajeLibro" class="msjFacturas"></div></td>
    <td>&nbsp;</td>
    <td width="15%" align="right">&nbsp;</td>
    <td width="13%" >&nbsp;</td>
  </tr>
  <tr>
    <td width="10%" align="right">Sucursal<span class="rojo">*</span>:</td>
    <td colspan="2">
    <select name="sucursal" id="sucursal" class="poscombo" 
     onchange="consultarAlmacenes()">
    ...
    <td width="8%" align="right">Factura:</td>
    <td valign="middle"><input type="text" name="facturas" id="facturas" style="width:140px"  
    disabled="disabled" value="<?php echo $ModificarT['numfactura'];?>" onkeypress="return soloEnteros(event)"/>
    <input type='checkbox' style="display:none;" name='facturado' id='facturado' onclick='solicitarNumFactura();' />
    </td>
    <td align="right"><input type='hidden' id='idTransaccion' value='<?php echo $ModificarT['idnotaventa'];?>' />
      Fecha:</td>
  ...
  <tr>
    ...
    <td><input type="text" name="ruta" id="ruta" style="width:140px;" value="<?php echo $datoCliente['nombre'];?>"
     disabled="disabled"/>
    <input type="hidden" name="idruta" id="idruta" />
    </td>
    <td align="right">Fecha de Entrega:</td>
    <td> 
      <input name="fechaentrega" type="text" id="fechaentrega" style="border:solid 1px #999;" size="10" class="date" value="<? 
	   if (isset($_GET['idnotaventa'])) {
  ...
  <tr>
    ...
    <td><input type="text" name="documento" id="documento" style="width:52%" onkeypress="return soloNumeros(event);" value="<?php echo $datoCliente['nit'];?>"/></td>
    <td align="right">Moneda:</td>
    <td>      
      <select name="moneda" id="moneda" onchange="limpiarDetalle();" class="poscombo">
      <?php          
  ...
  <tr>
    ...
    <td>
    <select name="tipoprecio" id="tipoprecio" class="poscombo">
      <?php
 ...
 <table width="100%" border="0">
   <tr>
    <td width="13%" align="right"><strong>Almacén:</strong></td>
    <td width="19%" align="left">
    <select name="idalmacen" id="idalmacen" style="width:130px;background:#FFF;border:solid 1px #999;">
     <?php 
	 if ($ModificarT['idsucursal'] != ""){
	   $sql = "select * from almacen where sucursal=$ModificarT[idsucursal]";
    ...
    <td width="12%" align="right"><strong>Producto:</strong></td>
    <td width="17%">
      <input type="text"  id="dato" autocomplete="off" onkeyup="consultorAutocompletar(event,this.id,this.value)"/><br>
    <div  id="resultados"  class="divresultado" >
        </div>
       <input type="hidden" id="codidproducto" />
        </td>
    <td width="29%"><div id="autoL1" class="autoLoading"></div></td>
    <td width="10%">T.C.: <?php
	if (isset($ModificarT['idnotaventa']))
  ...
  <div id="cuerpo" class="cuerpo">
     <table width="100%" border="0" id="tabla">
      <tr class="filadetalleui">
        ...
        <th width="82" style='display:none'>Id</th>
      </tr>           
      ...
      <tbody id="detalleT">
         ...
         $totalDNota = 0;
           if (isset($_GET['idnotaventa'])){
             $consulta =mysql_query("select p.idproducto,p.nombre
             ,round(ds.precio,2)as 'precio',date_format(ds.fechavencimiento,'%d/%m/%Y') 
             as 'fecha',ds.cantidad,ds.total as 'total',ds.lote,ds.unidadmedida
             ,ds.iddetalleingreso from detallenotaventa ds,producto p,notaventa s
              where ds.idnotaventa=s.idnotaventa and ds.idproducto=p.idproducto and s.idnotaventa=".$_GET['idnotaventa']);
              $fil = 0;
              $aux = 1;
              ...
              while($dato = mysql_fetch_array($consulta)){			 
                ...
                $totalDNota = $totalDNota + (round(($dato['precio']/$aux),4) *$dato['cantidad']);
                $color = '#F6F6F6' ;
                $fil++;				    
                $montoPrecio = round(($dato['precio']/$aux),4);
                $montoTotal = round(($dato['precio']/$aux),4) * $dato['cantidad'];
                ...
                echo "  <td align='center'>".number_format($montoPrecio,4)."</td>";
                echo "  <td align='center'>".number_format($montoTotal,4)."</td>";
     ...
     <div style="margin: 0 auto; width:100.2%; margin-top:2px;position:relative;">     
         <div id="pie_izq" class="pie_izq" >
          <table width="100%" border="0">
            <tr>
              ...
              <textarea id="glosa" style="width:90%; height:50px;" ><?php echo $ModificarT['glosa'];?></textarea></td>
            </tr>
            <tr>
              <td height="35">&nbsp;</td>
            </tr>
        ...
        <div id="pie_centro" class="pie_centro" >
          <table width="99%">
             <tr class="filadetalleui">
               <td colspan="3" align="center">Forma de Pago</td>
               </tr>
             <tr >
               <td align="right">&nbsp;</td>
               <td>&nbsp;</td>
             ...
             <tr >
               <td width="28%" align="right">Efectivo:</td>
               <td width="25%">
               <?php			 
			  ...
			  ?>              
                 ...
                 <input type="text" id="efectivo" name="efectivo" style="width:60px;"
                  onkeyup="calcularCambio2();" onkeypress="return soloNumeros(event);" />               </td>
               <td width="47%"><select id="caja" name="caja" <?php echo $atributo;?>   
		  ...
		  ?>
             ...
             <tr >
               ...
               <td><input type="text" id="cambio" name="cambio" style="width:60px;"
                value="<?php echo $ModificarT['credito'];?>" disabled="disabled"/></td>
               <td><select name="diascredito" id="diascredito" <?php echo $atributo;?> onchange="calcularCambio()"
 ...
 <table width="99%" cellspacing="1">
  <tr >
    ...
    <td width="32%"><div align="right">Sub Total:</div></td>
    <td colspan="2" align="left">
    <input id="subtotal" name="subtotal" style="width:70%;" readonly="readonly" value="<?php echo $totalDNota;?>"/>
    </td>
    </tr>
  <tr >
    <td><div align="right"></div></td>
    <td ><div align="right">Descuento:</div></td>
    <td width="23%" ><input id="pdescuento" name="pdescuento" style="width:60%;" onkeyup="calcularDescuento();" 
    onkeypress="return soloNumeros(event);" value="<?php echo $ModificarT['descuento'];?>"/>%</td>
    <td width="42%"><input id="descuento" name="descuento" style="width:60px;" readonly="readonly"/></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td align="right">Recargo:<div id="totalbs"></div></td>
    <td><input id="precargo" name="precargo" style="width:60%;" onkeypress="return soloNumeros(event);"
     onkeyup="calcularRecargo()" value="<?php echo $ModificarT['recargo'];?>"/>%</td>
