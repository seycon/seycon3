<?php
	...
	//$idtransaccion = 1;
	$idtransaccion = $_GET['idingreso'];
	$sql = "select i.idingreso as 'numero',nombrepersona
	,date_format(fecha,'%d/%m/%Y')as 'fecha',recibo,cheque,left(p.cuenta,25)as 'cuenta',
	left(s.nombrecomercial,25)as 'nombrecomercial',i.idpersona
	,i.tipopersona,left(concat(t.nombre,' ',t.apellido),25)as 'usuario'
	,i.tipocambio,i.glosa from ingreso i,sucursal s,plandecuenta p,usuario u,trabajador t
	 where i.idsucursal=s.idsucursal 
	 and i.cuenta=p.codigo 
	 and u.idusuario=i.idusuario 
	 and u.idtrabajador=t.idtrabajador 
	 and idingreso=$idtransaccion;";
	...
	switch ($datosGenerales['tipopersona']) {
	 ...
	 break;
	 case 'trabajador':
	  $sql = "select left(concat(nombre,' ',apellido),40)as 'nombre' from trabajador
	   where idtrabajador=$datosGenerales[idpersona]";
	 break;	
	}
	...
	$sql = "select left(p.cuenta,25)as 'cuenta',left(d.descripcion,55)as 'descripcion',d.montobolivianos,d.montodolares 
	 from detalleingreso d,plandecuenta p where d.idcuenta=p.codigo 
	 and p.estado=1 and idingreso=$idtransaccion order by iddetalleingreso;";
	...
	{	
	  ...
	  <tr>
		...
		<td class='session2_titulos'>Recibo:</td>
...
<link rel="stylesheet" href="ingresos.css" type="text/css" />
...
<div class="session1_contenedorTitulos">
   <table width="100%" border="0">
     <tr><td align="center" class="session1_titulo1"><?php echo $tituloPrincipal;?></td></tr> 
    </table>
</div>
