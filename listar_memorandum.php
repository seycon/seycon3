...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Memorándum','listar_memorandum.php','nuevo_memorandum.php');
...
function anular(){
 return confirm("Desea anular este Memorandum ?");
}


function eliminarTransaccion(){
  location.href = "anular_registro.php?idmemorandum="+$$("idDelete").value+"&tabla=memorandum&menu=13";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Memorandum?';
...
<form name="listado" id="listado" method='get' action='listar_memorandum.php#t13' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				   echo "<option value=idmemorandum>Nº</option>";
   	               echo "<option value=m.referencia>Referencia</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])){
		...
		$condicion = " where m.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and ".$_GET['campo']." like '"
			.$_GET['abuscar']."%' and $cFecha m.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= " where m.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and m.estado = ".$_GET['estado'];
		} else { 
			$condicion= ' where m.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and m.estado = 1';
		}
	}

	$sql = "SELECT idmemorandum,left(m.para,25),left(m.referencia,25),date_format(m.fecha,'%d/%m/%Y'),left(m.cc,25)
	,left(concat(t.nombre,' ',t.apellido),25) from memorandum m,usuario u,trabajador t "
	.$condicion." order by idmemorandum desc";
...
<?php if ($numeroPagina > 0) { ?>
  <a href="<?php printf("%s?numPage=%d%s#t13", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>

</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t13", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
 <?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t13", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
        <a href="<?php printf("%s?numPage=%d%s#t13", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>">
...
<?php
$nombres = array(  "idmemorandum", "para", "referencia", "fecha", "cc" ,"Responsable" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_memorandum.php?idmemorandum=".$id."&sw=1#t13' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
...
if ($fileAcceso['Eliminar'] == "Si"){ 
 ...
 echo "<td><div align='center'><a href='memorandum/imprimir_memorandum.php?idmemorandum=".$id."' target='_blank' onclick='return imprimir(this)'><img src='css/images/imprimir.gif' title='Imprimir' alt='imprimir' border='0' /><br /></a></div></td>";
