...
<?php
    ...
    $fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Recibo de Ingreso','nuevo_reciboingreso.php'
	,'listar_reciboingreso.php');
	 ...
	 $sql = "select left(tituloreciboingr,30)as 'tituloreciboingr' from impresion where idimpresion = 1";
     $tituloPrincipal = $db->getCampo('tituloreciboingr', $sql);
  	 $transaccion = "insertar";
     
	 if (isset($_GET['idingreso'])){
		$sql = "select *from reciboingreso where idreciboingreso=".$_GET['idingreso']; 
...
<script src="reciboingreso/NRecibo.js"></script>
...
<td colspan="3" align='right'>
        ...
        <table width="356" border="0">
          <tr>
            ...
            <td width="142">
			<?php $idreciboingreso=0; 
			if (isset($_GET['idingreso'])) {
				$idreciboingreso = $_GET['idingreso'];
       ...
       <div id="datos_factura" >
            <table width="100%" border="0">
                ...
                <tr>
                  ...
                  <td><input type="checkbox" name="pagado" id="pagado" <?php if(isset($_GET['idingreso'])) 
                ...
                <tr>
                  ...
                  <td><select name="receptor" id="receptor"  onchange="cambiarDependencias();" 
                  ...
                  value="<?php if (isset($maestro['idrecibido'])){
	                 echo $maestro['nombrerecibido'];
	               }
	              ?>" autocomplete="off"/>
                    <div id="cliente" class="divresultado"></div>
                  <input type="hidden" id="idpersonarecibida" value="<?php echo $maestro['idrecibido'];?>" />              
                  
                  </td>
                  <td><div id="autoL1" class="autoLoading"></div></td>
                  <td><div align="right">Fecha:</div></td>
                  <td><label for="receptor">
                    <input name="fecha" type="text" id="fecha" value="<?php if(isset($_GET['idingreso'])) 
					echo $db->GetFormatofecha($maestro['fecha'],'-'); else  echo date("d/m/Y"); ?>" size="10"  />
                  </label></td>
                  <td><div align="right">Firma Digital:</div></td>
                  <td><input type="checkbox" name="firmadigital" id="firmadigital" <?php if(isset($_GET['idingreso'])) 
    ...
    <table width="100%" border="0">
      <tr>
        ...
        <td width="18%">
          <input type="text" name="importe" id="importe" onkeyup="enter('input',event);"
           onkeypress="return soloNumeros(event)"  /></td>
        <td width="8%"></td>
        <td width="17%">
         <input type="button" onclick="insertarNewItem('a');" name="agregar" id="agregar" value="Registrar [Enter]"
      ...
      <div id="cuerpo" class="cuerpo">
          <table width="100%" border="0" id="tabla">
			...
			$total = 0;
			  if (isset($_GET['idingreso'])){
				$sql = "select descripcion,ingreso from detallereciboingreso where idreciboingreso=".$_GET['idingreso'];
				$detalle = mysql_query($sql);
				$i = 0;				
				 while($datos = mysql_fetch_array($detalle)){
				  $total = $total + $datos['ingreso'];	 
				$i++;	 
				   echo "<tr>
				   ...
				   <td align='center'>".number_format($datos['ingreso'],2)."</td>
				   </tr>";					
				 } 
			  }			
			?>
...
<table width="100%" border="0">
  <tr>
    ...
    <td width="22%">
    <input type="text" name="total_ingreso" id="total_ingreso" readonly="readonly" value="<?php echo number_format($total,2);?>" />
    </td>
  </tr>
</table>        
</div>
