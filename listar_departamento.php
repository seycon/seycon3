...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Departamento','listar_departamento.php','nuevo_departamento.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?iddepartamento="+$$("idDelete").value+"&tabla=departamento&menu=4";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Departamento?';
...
<div class="cabeceraFormulario">
    <div class="menuTituloFormulario"> Recursos > Departamento </div>
    <div class="menuFormulario"> 
     <?php
       $estructura = $_SESSION['estructura'];
       $menus = $estructura['Recursos'];
       $privilegios = $db->getOpciones($menus, "Departamento"); 
       $option = "";
...
<form name="listado" id="listado" method='get' action='listar_departamento.php#t4' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=iddepartamento>Nº</option>";
				echo "<option value=d.nombre>Nombre</option>";
...
<?php
	...
	if ($_GET['abuscar'] != '') {
    $condicion = " where d.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and ".$_GET['campo']." like '"
	    .$_GET['abuscar']."%' and d.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion = " where d.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and d.estado = ".$_GET['estado']; 
		} else {
			$condicion = ' where d.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and d.estado = 1';
		}
	}

	$sql = "SELECT iddepartamento,left(d.nombre,20),left(d.descripcion,40),left(concat(t.nombre,' ',t.apellido),25)"
	    ." from departamento d,trabajador t,usuario u ".$condicion." order by iddepartamento desc";
...
<?php
	$nombres = array(  "Iddepartamento", "Nombre", "Descripción", "Responsable");
	...
	if (isset($_GET['estado']) && ($_GET['estado'] == 0)){	
	   ...
	   if ($fileAcceso['Modificar'] == "Si") { 
		 echo "<td><div align='center'><a href='nuevo_departamento.php?iddepartamento=".$id."&sw=1#t4'
		  onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar'
		   alt='editar' border='0' /><br /></a></div></td>";
	   } else {
		 echo "<td><div align='center'><img src='css/images/edit.gif'
		  title='Sin Privilegios para Modificar' alt='editar' border='0' /></div></td>";	
	   }
