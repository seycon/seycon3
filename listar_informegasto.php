...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Informes de Gastos','listar_informegasto.php','nuevo_informegastos.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idinformegasto="+$$("idDelete").value+"&tabla=informegasto&menu=3";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Informe de Gastos?';
...
function accionPostRegistro(){
   window.open('informegastos/imprimir_informeGastos.php?idinforme='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_informegasto.php#t3' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idinformegasto>Nº</option>";
				echo "<option value=nrodocumentos>Nº Documento</option>";
...
$sql = "SELECT idinformegasto,nrodocumentos,date_format(fecha,'%d/%m/%Y'),
round(montorendicion,2),left(concat(t.nombre,' ',t.apellido),25)  from informegasto i,usuario u,trabajador t 
 ".$condicion." order by idinformegasto  desc";
...
<?php
$nombres = array(  "Nº", "Nº Documentos", "fecha" , "monto Rendición", "Responsable");
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_informegastos.php?idinforme=$id#t3'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
