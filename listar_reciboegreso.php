...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Recibo de Egreso','listar_reciboegreso.php','nuevo_reciboegreso.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idreciboegreso="+$$("idDelete").value+"&tabla=reciboegreso&menu=2";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Recibo Egreso?';
...
<form name="listado" id="listado" method='get' action='listar_reciboegreso.php#t2' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idreciboegreso>Nº</option>";
				echo "<option value=nombreentregado>Nombre Entregado</option>";
...
if (isset($_GET['abuscar'])){
  ...
  $condicion = " where ".$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha estado=".$_GET['estado'];
}
else 
...
$sql = "select idreciboegreso,left(nombreentregado,25),date_format(fecha,'%d/%m/%Y'),left(responsable,25),round(totalegreso,2) from reciboegreso ".$condicion." order by idreciboegreso desc";
...
<?php
$nombres = array(  "Nº", "Entregamos a", "Fecha", "Responsable", "Total Egreso" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
    ...
    if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_reciboegreso.php?idegreso=".$id."#t2'>
 ...
 echo "<td><div align='center'><a href='reciboegreso/imprimir_reciboEgreso.php?idrecibo=".$id."'
  target='_blank' onclick='return imprimir(this)'><img src='css/images/imprimir.gif'
   title='Imprimir' alt='imprimir' border='0' /><br /></a></div></td>";
