<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Agenda'], 'Recibo de Ingreso'
	,'listar_reciboingreso.php', 'nuevo_reciboingreso.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idreciboingreso="+$$("idDelete").value+"&tabla=reciboingreso&menu=1";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Recibo Ingreso?';
...
<div class="cabeceraFormulario">
  <div class="menuTituloFormulario"> Agenda > Recibo de Ingreso </div>
  <div class="menuFormulario"> 
   <?php
     $estructura = $_SESSION['estructura'];
     $menus = $estructura['Agenda'];
     $privilegios = $db->getOpciones($menus, "Recibo de Ingreso"); 
     $option = "";
...
<form name="listado" id="listado" method='get' action='listar_reciboingreso.php#t1' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idreciboingreso>Nº</option>";
				echo "<option value=nombrerecibido>Nombre Recibido</option>";
...
if (isset($_GET['abuscar'])){
  ...
  $condicion = " where $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and estado=".$_GET['estado'];
}
else 
...
$sql = 'select idreciboingreso,left(nombrerecibido,25),fecha,left(responsable,25),round(totalingreso,2) from reciboingreso '.$condicion.' order by idreciboingreso desc';
...
<?php
	$nombres = array(  "Nº", "Recibimos de", "Fecha", "Responsable", "Total Ingreso" );
    ...
    if ($fileAcceso['Modificar'] == "Si"){ 
	  echo "<td><div align='center'><a href='nuevo_reciboingreso.php?idingreso=".$id."#t1'>
...
echo "<td><div align='center'><a href='reciboingreso/imprimir_reciboIngreso.php?idrecibo=".$id."' target='_blank'><img src='css/images/imprimir.gif' title='Imprimir' alt='imprimir' border='0' /><br /></a></div></td>";
