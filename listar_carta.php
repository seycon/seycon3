...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Carta','listar_carta.php','nuevo_carta.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idcarta="+$$("idDelete").value+"&tabla=carta&menu=14";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Carta?';
...
<form name="listado" id="listado" method='post' action='listar_carta.php#t14' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
					echo "<option value=idcarta>Nº</option>";
                   	echo "<option value=m.para>Para</option>";
...
if (isset($_POST['abuscar'])){
  ...
  $condicion = " where m.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and ".$_POST['campo']." like '".$_POST['abuscar']."%' and $cFecha m.estado=".$_POST['estado'];
}
else 
if (isset($_POST['estado'])) 
$condicion= " where m.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and m.estado = ".$_POST['estado'];
else 
$condicion= ' where m.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and m.estado = 1';

$sql = "SELECT idcarta,left(m.para,25),left(m.referencia,25),date_format(m.fecha,'%d/%m/%Y'),left(m.tipo,25),left(concat(t.nombre,' ',t.apellido),25) from carta m,usuario u,trabajador t ".$condicion." order by idcarta desc";
...
$nombres = array(  "idmemorandum", "para", "referencia", "fecha", "Tipo" ,"Responsable" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_carta.php?idcarta=".$id."&sw=1#t14' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
...
if ($fileAcceso['Eliminar'] == "Si"){ 
	...
	echo "<td><div align='center'><a href='carta/imprimir_carta.php?idcarta=".$id."' target='_blank' onclick='return imprimir(this)'><img src='css/images/imprimir.gif' title='Imprimir' alt='imprimir' border='0' /><br /></a></div></td>";
		for( $i = 0 ; $i <=5 ; $i++ ){
		if($fila[$i] == "")
		{
