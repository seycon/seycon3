...
<?php
	...
	,'listar_cuentaporcobrar.php','nuevo_cuentaporcobrar.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idporcobrar="+$$("idDelete").value+"&tabla=porcobrar&menu=7&reg=cuentaporcobrar&libro=Cuenta por cobrar&tipolibro=egreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Cuenta Por Cobrar?';
...
function accionPostRegistro(){
   window.open('cuentaporcobrar/imprimir_cuenta_cobrar.php?idcuenta='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_cuentaporcobrar.php#t7' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idporcobrar>Nº</option>";
	            echo "<option value=s.nombrecomercial>Sucursal</option>";	
	            echo "<option value=t.nombre>Deudor</option>";
...
<?php
	...
	$sql = "
	(SELECT idporcobrar,c.numerocuenta,left(s.nombrecomercial,25)
	,date_format(fecha,'%d/%m/%Y'),monto,c.moneda,t.nombre,c.tipocambio 
	,left(concat(tb.nombre,' ',tb.apellido),20) 
	from cuentaporcobrar c,cliente t,sucursal s,usuario u,trabajador tb  
	...
	union (
	SELECT idporcobrar,c.numerocuenta,left(s.nombrecomercial,25)
	,date_format(fecha,'%d/%m/%Y'),monto,
	c.moneda,t.nombre,c.tipocambio 
	,left(concat(tb.nombre,' ',tb.apellido),20) 
	from cuentaporcobrar c,trabajador t,sucursal s,usuario u,trabajador tb  
	where c.iddeudor=t.idtrabajador 
	and s.idsucursal=c.idsucursal 
	and c.idusuario=u.idusuario 
	and u.idtrabajador=tb.idtrabajador 
	...
	union(
	SELECT idporcobrar,c.numerocuenta,left(s.nombrecomercial,25)
	,date_format(fecha,'%d/%m/%Y'),monto,c.moneda
	,t.nombre,c.tipocambio 
	,left(concat(tb.nombre,' ',tb.apellido),20) 
	from cuentaporcobrar c,proveedor t,sucursal s,usuario u,trabajador tb  
	where c.iddeudor=t.idproveedor 
	and s.idsucursal=c.idsucursal 
	and c.idusuario=u.idusuario 
	and u.idtrabajador=tb.idtrabajador 
	and c.tipodeudor='proveedor' 
	$condicion) ORDER BY idporcobrar DESC";
...
<?php
	$nombres = array(  "Nº", "Sucursal","Fecha", "Monto","Moneda" , "Deudor", "Usuario");
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
    ...
    if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_cuentaporcobrar.php?idporcobrar=".$id."&sw=1#t7'
	  onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar'
	   alt='editar' border='0' /><br /></a></div></td>";
	} else {
	...
	if ($fileAcceso['Eliminar'] == "Si") { 
		 echo "<td><div align='center' class='cursorDelete'><img src='css/images/borrar.gif'
		  title='Anular' alt='borrar' border='0' onclick='openMensaje($id)'/><br /></div></td>";
		} else {
