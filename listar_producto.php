...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Tipos de Productos','listar_producto.php','nuevo_productos.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idproducto="+$$("idDelete").value+"&tabla=producto&menu=6";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Producto?';
...
<form name="listado" id="listado" method='get' action='listar_producto.php#t6' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idproducto>Nº</option>";
				echo "<option value=p.nombre>Nombre</option>";
				echo "<option value=s.nombre>Sub Grupo</option>";
				echo "<option value=p.unidaddemedida>U.M.</option>";
				echo "<option value=p.unidadalternativa>U.A.</option>";		
				echo "<option value=p.stockminimo>Stock Mínimo</option>";  
			  ?>
            </select>
   </div> 
...
<tr>
<td width='227' ><? if (isset($_GET['estado']) && ($_GET['estado'] == 0))  echo '';?></td>
<td width='249' style='font-family: Verdana, Geneva, sans-serif;'> 
...
if ($_GET['abuscar'] != ''){
$condicion= " where p.idsubgrupo=s.idsubgrupo and ".$_GET['campo']." like '".$_GET['abuscar']."%' and p.estado=".$_GET['estado'];
}
else 
if (isset($_GET['estado'])) 
$condicion= " where p.idsubgrupo=s.idsubgrupo and p.estado = ".$_GET['estado'];
else 
$condicion= ' where p.idsubgrupo=s.idsubgrupo and p.estado =1';



$sql = 'select p.idproducto,p.nombre,s.nombre,p.unidaddemedida,p.unidadalternativa,p.conversiones,p.stockminimo,round(p.costo,2)   
from producto p,subgrupo s '.$condicion.' order by idproducto desc';
...
$nombres = array(  "Nº", "Nombre", "Sub Grupo", "U.M.", "U.A.", "Conv.", "Stock Mínimo", "Costo" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_productos.php?idproducto=".$id."&sw=1#t6' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
