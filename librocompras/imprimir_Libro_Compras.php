...
<p><br>  
 ...
 $header = '        	
     ...
     <td colspan="8" align="center">      
	   <div style="margin:0 auto;text-align:center;font-size:24px;font-weight:bold;">LIBRO DE COMPRAS I.V.A.</div>
	 </td>
     </tr>
   </thead> 
  <tr>
  ...
  <tr>
    ...
    <td align="center"><div style="font-size:11px">{PAGENO}/{nb}</div></td>
  </tr>
   <tr>
...
<?php
	...
	$recuentoDebito = 0;
	$recuentoFactura = 0;  
	$sql = "select idlibrocomprasiva,day(fechadeemision)as 'dia',Month(fechadeemision)as
		   'mes', year(fechadeemision)as       'anio',fechadeemision,numdenitproveedor
		   ,left(nomrazonsociprove,29)as 'nomrazonsociproves',numfacturaopoliza,
		  numautorizacion,codigodecontrol,round(totalfactura,2)as 'totalfactura'
		  ,round(totalice,2)as 'totalice',round(importeexcento,2)as 'importeexcento'
		  ,round(importeneto,2)as 'importeneto',round(creditofiscal,2)as 'creditofiscal' 
		  from librocomprasiva where Month(fechadeemision)=$numMes 
		  and year(fechadeemision)=$numAnio and  folio=$idsucursal 
		  and estado=1 order by fechadeemision asc ";
	$consulta = mysql_query($sql);
	$contadorFilas = 0;
...
<thead>
  <tr>
    ...
    <td width="8%"  rowspan="2" class="tablacabecera"  >Nº DE NIT PROVEEDOR</td>
    <td width="21%"  rowspan="2" class="tablacabecera" >NOMBRE O RAZON SOCIAL PROVEEDOR</td>
    <td width="8%"  rowspan="2" class="tablacabecera"  >Nº DE FACTURA/POLIZA</td>
	...
	while ($dato = mysql_fetch_array($consulta)) {
		$i =$i +1 ;
		    $contadorFilas = $contadorFilas + 1;
          	$indiceRecorrido = $dato['idlibrocomprasiva'];	
	        ...
	        $recuentoDebito = $recuentoDebito + $dato['creditofiscal'];
			$recuentoFactura = $recuentoFactura + $dato['totalfactura'];

		
		if ($i == 31 || $contadorFilas == $cant) {
			echo "<tr >";
			echo "<td  style='font-size:8px;border-left:solid 1px #000;
			border-bottom:solid 1px          #000;'>".aumentarDigito($dato['dia'])."</td>";
			echo "<td  style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px          #000;border-left-style:dotted'>".aumentarDigito($dato['mes'])."</td>";
			echo "<td style='font-size:10px;border-left:solid 1px #000;border-bottom:solid 1px #000;
			border-left-style:dotted'>".$dato['anio']."</td>";
			echo "<td style='font-size:10px;border-left:solid 1px #000;border-bottom:solid 1px #000;
			border-left-style:dotted'>".$dato['numdenitproveedor']."</td>";
			echo "<td   style='text-align:left;font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>". strtoupper($dato['nomrazonsociproves'])."</td>";
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>".$dato['numfacturaopoliza']."</td>";
			echo "<td style='font-size:10px;border-left:solid 1px #000;border-bottom:solid 1px #000;
			border-left-style:dotted'>".$dato['numautorizacion']."</td>";
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>".$dato['codigodecontrol']."</td>";		
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>".convertir($dato['totalfactura'])."</td>";				
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>".convertir($dato['totalice'])."</td>";	
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>".convertir($dato['importeexcento'])."</td>";	
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-bottom:solid 1px #000;border-left-style:dotted'>".convertir($dato['importeneto'])."</td>";	
			echo "<td style='font-size:10px;border-left:solid 1px #000;
			border-right:solid 1px #000;border-bottom:solid 1px #000;
			border-left-style:dotted'>".convertir($dato['creditofiscal'])."</td>";	
			echo "</tr>";
			break;			
		} else {
			echo "<tr >";
			...
			echo "<td class='punteado'>".$dato['numdenitproveedor']."</td>";
			echo "<td  class='punteado' style='text-align:left;'>".strtoupper($dato['nomrazonsociproves'])."</td>";
			echo "<td class='punteado'>".$dato['numfacturaopoliza']."</td>";
			...
			echo "<td class='punteado' style='border-right:solid 1px #000;'>".convertir($dato['creditofiscal'])."</td>";	
			echo "</tr>";
		}
...
<div style="width:4%; font-size:9px; position:absolute;left:82%;top:10%;text-align:center;
 border:1px solid #000;"><?php echo $numAnio;?></div>
...
<div style="width:7%; font-size:9px; position:absolute;left:88%;top:10%; text-align:center;
