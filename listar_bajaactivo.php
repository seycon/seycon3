...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Activo'],'Baja de Activo','listar_bajaactivo.php','nuevo_bajaactivo.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idbajaactivo="+$$("idDelete").value+"&tabla=bajaactivo&menu=2&libro=Baja Activo&tipolibro=egreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Baja Activo?';
...
function accionPostRegistro(){
   window.open('activo/imprimir_bajaactivo.php?idactivo='+$$("idDelete").value,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_bajaactivo.php#t2' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=ba.numero>Nº</option>";
				echo "<option value=a.nombre>Activo</option>";
				echo "<option value=s.nombrecomercial>Sucursal</option>";
				echo "<option value=ba.motivobaja>Motivo Baja</option>";		  
			  ?>
            </select>
   </div> 
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		$cFecha = ($_GET['fecha'] == "") ? "" : " ba.fechabaja ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";
		$condicion = " where ba.idactivo=a.idactivo and ba.idsucursal=s.idsucursal and  $cFecha ".$_GET['campo']
		." like '".$_GET['abuscar']."%' and ba.estado=".$_GET['estado'];
	} else { 
		if (isset($_GET['estado'])) {
			$condicion = " where ba.idactivo=a.idactivo and ba.idsucursal=s.idsucursal and ba.estado = ".$_GET['estado'];
		} else { 
			$condicion = " where ba.idactivo=a.idactivo and ba.idsucursal=s.idsucursal and ba.estado = 1";
		}
	}

	$sql = "SELECT idbajaactivo,ba.numero,a.nombre,s.nombrecomercial,ba.motivobaja,date_format(ba.fechabaja,'%d/%m/%Y'),
	ba.cantidad from bajaactivo ba,activo a,sucursal s ".$condicion." order by idbajaactivo desc";
...
<?php
$nombres = array(  "Nº", "Activo", "Sucursal","Motivo Baja","Fecha","Cantidad");
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_bajaactivo.php?idbajaactivo=".$id."&sw=1#t2' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
