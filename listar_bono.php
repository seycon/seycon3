...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Planilla de Bonos','listar_bono.php','bonos.php');
...
<script language="javascript">
  ...
  function eliminarTransaccion(){
	location.href = "anular_registro.php?idbono="+$$("idDelete").value+"&tabla=bono&menu=4&libro=Bono&tipolibro=egreso";
  }
  ...
  var verificarEstado = function(){
	var filtro = "transaccion=referenciaMaestro&idbono="+$$("idDelete").value;
	consultar("bonos/DBonos.php",filtro,mostrarVentana);
  }
  ...
  var mostrarVentana = function(resultado){
   if (resultado <= 0){
    ...
    $$("modal_contenido").innerHTML = '¿Desea anular este Bono Mensual?';
	$$("modal_boton2").style.visibility = "visible"; 
    $$("modal_mensajes").style.visibility = "visible";
    $$("overlay").style.visibility = "visible"; 	
   }else{
	$$("modal_tituloCabecera").innerHTML = 'Advertencia';
    $$("modal_contenido").innerHTML = 'No es posible anular debido a que ya se generó la planilla de sueldos.';
    $$("modal_mensajes").style.visibility = "visible";
    $$("overlay").style.visibility = "visible"; 
...
<form name="listado" id="listado" method='get' action='listar_bono.php#t4' enctype='multipart/form-data'>
...
<?php
	...
	if ($_GET['abuscar'] != '') {
	  if ($_GET['campo'] == "month" || $_GET['campo'] == "year") {	 
          $condicion = " b.estado=".$_GET['estado']." and $_GET[campo](b.fecha)=".$_GET['abuscar'];
	  } else {
		  $condicion = " b.estado=".$_GET['estado']." and ".$_GET['campo']." like '".$_GET['abuscar']."%'";
	  }
	} else {
		if (isset($_GET['estado'])) {
			$condicion = " b.estado = ".$_GET['estado']; 
		} else {
			$condicion = ' b.estado = 1';
		}
	}

    $sql = "select b.idbono,month(b.fecha),year(b.fecha),left(s.nombrecomercial,30)
	,left(concat(t.nombre,' ',t.apellido),35)as 'usuario' 
	from bono b,trabajador t,usuario u,trabajador tb,sucursal s where b.idusuario=u.idusuario 
    and u.idtrabajador=t.idtrabajador and b.idtrabajador=tb.idtrabajador 
	and tb.idsucursal=s.idsucursal and ".$condicion." group by s.idsucursal,year(b.fecha),month(b.fecha) ";
...
<?php
	$nombres = array(  "Mes", "Año", "Sucursal", "Usuario");
...
if ($fileAcceso['Eliminar'] == "Si") { 
	 ...
	 <img src='css/images/borrar.gif' title='Anular' alt='borrar' border='0' onclick='openMensaje($id)'/><br /></div></td>";
	}else{
	 echo "<td><div align='center'>
	 <img src='css/images/borrar.gif'  title='Sin Privilegios para Anular' alt='borrar' border='0' /><br /></div></td>";	
	}
}
	...
	for( $i = 1; $i <= 4; $i++ ) {
		...
		else
		{
         switch($i){
			case 1:
			  echo "<td class='centro'>".$db->mes($fila[$i])."</td>";
			break;
			default:
