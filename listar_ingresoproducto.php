...
$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Ingresar Productos','listar_ingresoproducto.php','nuevo_ingresoalmacen.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idingresoprod="+$$("idDelete").value+"&tabla=ingresoprod&menu=3&reg=ingresoproducto&libro=Ingreso Almacen&tipolibro=ingreso";
}
...
var verificarEstado = function(){
	var filtro = "transaccion=referenciaMaestro&idingreso="+$$("idDelete").value;
  	consultar("ingresoalmacen/DIngreso.php",filtro,mostrarVentana);
}

var mostrarVentana = function(resultado){
   if (resultado <= 0){
    $$("modal_tituloCabecera").innerHTML = 'Advertencia';
    $$("modal_contenido").innerHTML = '¿Desea anular este Ingreso de Producto?';
	$$("modal_boton2").style.visibility = "visible"; 
    $$("modal_mensajes").style.visibility = "visible";
    $$("overlay").style.visibility = "visible"; 	
   }else{
	$$("modal_tituloCabecera").innerHTML = 'Advertencia';
    $$("modal_contenido").innerHTML = 'No es posible anular debido a que ya se realizaron movimientos con el ingreso.';
    $$("modal_mensajes").style.visibility = "visible";
    $$("overlay").style.visibility = "visible"; 
...
function accionPostRegistro(){
   window.open('ingresoalmacen/imprimir_ingresoProductos.php?idingreso='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_ingresoproducto.php#t3' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				  echo "<option value=p.nroingresoprod>Nº</option>";
				  echo "<option value=a.nombre>Almacén</option>";
				  echo "<option value=p.facproveedor>Doc.</option>";
				  echo "<option value=p.monto>Monto</option>";
...
if (isset($_GET['abuscar'])){
 $cFecha = ($_GET['fecha'] == "") ? "" : " p.fecha ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";	
 $condicion= "  a.idalmacen = p.idalmacen and $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and p.estado=".$_GET['estado'];
}
else{ 
 if (isset($_GET['estado'])) 
  $condicion= "  a.idalmacen = p.idalmacen and p.estado = ".$_GET['estado'];
 else 
 $condicion= '  a.idalmacen = p.idalmacen and p.estado =1';
}


$sql = "SELECT p.nroingresoprod, p.fecha, left(a.nombre,15)as almacen, left(IF( p.caja !=  '', (
SELECT pc.cuenta
FROM plandecuenta pc
WHERE pc.codigo = p.caja
AND pc.estado =1
), (

SELECT pc.cuenta
FROM plandecuenta pc
WHERE pc.codigo = p.cuentacontable
AND pc.estado =1
) ),25) AS  'cuentacontable'
, p.facproveedor, round(p.monto,2)as monto, left(concat(t.nombre,' ',t.apellido),25)as responsable,p.idingresoprod  
        from ingresoproducto p,almacen a,usuario u,trabajador t where u.idusuario=p.idusuario 
		and u.idtrabajador=t.idtrabajador and p.tipo='I' and ".$condicion." order by idingresoprod desc";
...
$nombres = array(  "idingresoprod",  "Fecha",  "Almacén", "Cuenta Contable" , "Doc.", "monto","Usuario" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_ingresoalmacen.php?idingresoprod=".$id."&sw=1#t3' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
