...
$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Combinación','listar_combinacion.php','nuevo_combinacion.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idcombinacion="+$$("idDelete").value+"&tabla=combinacion&menu=11";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Combinación?';
...
<form name="listado" id="listado" method='get' action='listar_combinacion.php#t11' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=c.idcombinacion>Nº</option>";
				echo "<option value=c.nombre>Nombre Combinación</option>";
				echo "<option value=t.nombre>Tipo Combinación</option>";			  
			  ?>
            </select>
   </div> 
...
<?php
	...
	if ($_GET['abuscar'] != '') {
	    $condicion = " where ".$_GET['campo']." like '".$_GET['abuscar']."%' and c.estado=".$_GET['estado'];
	} else { 
	    if (isset($_GET['estado'])) {
	        $condicion = " where c.estado = ".$_GET['estado'];
		} else { 
	        $condicion = ' where c.estado =1';
		}
	}

	$sql = 'SELECT c.idcombinacion,c.nombre,left(t.nombre,25),round(c.total,2)
	,left(c.glosa,30) from combinacion c,tipocombinacion t '
	.$condicion.' and t.idtipocombinacion=c.idtipocombinacion order by c.idcombinacion desc';
...
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
        <a href="<?php printf("%s?numPage=%d%s#t10", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
$nombres = array(  "Nº", "Nombre Combinación", "Tipo Combinación", "Total", "Glosa" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_combinacion.php?idcombinacion=".$id."&sw=1#t11' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
