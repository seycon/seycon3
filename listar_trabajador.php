...
<?php
	if (!isset($_SESSION)) {
	 ...
	 $db = new MySQL();
	  ...
	  $fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Trabajadores','listar_trabajador.php','nuevo_trabajador.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idtrabajador="+$$("idDelete").value+"&tabla=trabajador&menu=2";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Trabajador?';
...
function accionPostRegistro() {
   window.open('trabajador/imprimir_trabajadores.php?idtrabajador='+$$("idDelete").value
...
<form name="listado" id="listado" method='get' action='listar_trabajador.php#t6' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idtrabajador>Nº</option>";
				echo "<option value=t.nombre>Nombre</option>";
				echo "<option value=carnetidentidad>C.I.</option>";
...
<?php
	...
	if ($_GET['abuscar']!='') {
        $condicion= " and ".$_GET['campo']." like '".$_GET['abuscar']."%' and t.estado=".$_GET['estado'];		
    } else {
	...
	$sql = "SELECT idtrabajador,concat(nombre,' ',apellido)as nombre,carnetidentidad,c.cargo,sexo
	,s.nombrecomercial from trabajador t,cargo c,sucursal s 
	where t.idcargo = c.idcargo and t.idsucursal=s.idsucursal ".$condicion." order by idtrabajador desc";
...
<?php
$nombres = array(  "Nº", "Nombre Completo", "C.I.", "Cargo", "Sexo", "Sucursal" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_trabajador.php?idtrabajador=".$id."&sw=1#t6' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
} else {
...
echo "<td><div align='center' class='cursorDelete'>
<img src='css/images/imprimir.gif' title='Imprimir' alt='imprimir' border='0' onclick='openVentanaImprimir($id)'/>
<br /></div></td>";
