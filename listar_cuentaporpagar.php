...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Cuenta por Pagar','listar_cuentaporpagar.php'
	,'nuevo_cuentaporpagar.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idporpagar="+$$("idDelete").value
  +"&tabla=porpagar&menu=6&reg=cuentaporpagar&libro=Cuenta por pagar&tipolibro=ingreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Cuenta Por Pagar?';
...
function accionPostRegistro(){
   window.open('cuentaporpagar/imprimir_cuenta_pagar.php?idcuenta='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_cuentaporpagar.php#t6' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
					echo "<option value=idporpagar>Nº</option>";
                	echo "<option value=s.nombrecomercial>Sucursal</option>";		
					echo "<option value=c.moneda>Moneda</option>";
					echo "<option value=t.nombre>Acreedor</option>";
					echo "<option value=tb.nombre>Usuario</option>";
			  ?>
            </select>
   </div> 
...
<?php
  ...
  if (isset($_GET['abuscar'])) {
	...
	$condicion = " and ".$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha c.estado=".$_GET['estado'];
  } else {
	  if (isset($_GET['estado'])) {
		$condicion= " and c.estado = ".$_GET['estado'];
	  } else {
		$condicion= ' and c.estado = 1';
	  }
  }

	$sql = "(SELECT idporpagar,c.numerocuenta,left(s.nombrecomercial,25)
	,date_format(fecha,'%d/%m/%Y'),monto,c.moneda,t.nombre,c.tipocambio
	,left(concat(tb.nombre,' ',tb.apellido),20) 
	from cuentaporpagar c,sucursal s,cliente t,usuario u,trabajador tb 
	where c.iddeudor=t.idcliente 
	and s.idsucursal=c.idsucursal 
	and c.idusuario=u.idusuario 
	and u.idtrabajador=tb.idtrabajador 
	and c.tipodeudor='cliente' $condicion) 
	union (
	SELECT idporpagar,c.numerocuenta,left(s.nombrecomercial,25)
	,date_format(fecha,'%d/%m/%Y'),monto,c.moneda,t.nombre,c.tipocambio
	,left(concat(tb.nombre,' ',tb.apellido),20)
	 from cuentaporpagar c,trabajador t,sucursal s,usuario u,trabajador tb  
	where c.iddeudor=t.idtrabajador and s.idsucursal=c.idsucursal 
	and c.idusuario=u.idusuario 
	and u.idtrabajador=tb.idtrabajador 
	and c.tipodeudor='trabajador' $condicion) 
	union (
	SELECT idporpagar,c.numerocuenta,left(s.nombrecomercial,25
	),date_format(fecha,'%d/%m/%Y'),monto,c.moneda,t.nombre,c.tipocambio
	,left(concat(tb.nombre,' ',tb.apellido),20)
	 from cuentaporpagar c,proveedor t,sucursal s,usuario u,trabajador tb  
	where c.iddeudor=t.idproveedor and s.idsucursal=c.idsucursal 
	and c.idusuario=u.idusuario 
	and u.idtrabajador=tb.idtrabajador 
	and c.tipodeudor='proveedor' $condicion) ORDER BY idporpagar DESC";
...
<?php if ($numeroPagina > 0) { ?>
   <a href="<?php printf("%s?numPage=%d%s#t6", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t6", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
<?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t6", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t6", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
$nombres = array(  "Nº","Sucursal","Fecha", "Monto","Moneda" , "Acreedor", "Usuario");
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_cuentaporpagar.php?idporpagar=".$id."&sw=1#t6' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
...
if ($fileAcceso['Eliminar'] == "Si") { 
	 echo "<td><div align='center' class='cursorDelete'><img src='css/images/borrar.gif' title='Anular'
	  alt='borrar' border='0' onclick='openMensaje($id)'/><br /></div></td>";
	} else {
	 echo "<td><div align='center'><img src='css/images/borrar.gif'  title='Sin Privilegios para Anular'
	  alt='borrar' border='0' /><br /></div></td>";	
	}
}

echo "<td><div align='center' class='cursorDelete'><img src='css/images/imprimir.gif'
 title='Imprimir' alt='imprimir' border='0' onclick='openVentanaImprimir($id)'/><br /></div></td>";
	  ...
	  for( $i = 1 ; $i <= 7 ; $i++ ){
		...
		else
		{
          switch($i){
			case 1:
			  echo "<td class='centro'>".$fila[0]."</td>";
			break;   
			case 2:
			...
			case 4:
			  if ($fila[5] == "Dolares"){
				$monto =  round(($fila[$i] /  $fila[7]),2);
			  } else {
				 $monto = round($fila[$i],2); 
			  }			
			...
			break;
			case 7:
			  echo "<td class='centro'>".$fila[8]."</td>";
			break;
			default:
			  echo "<td >&nbsp;".$fila[$i]."</td>";   
		  }		
