...
$fileAcceso = $db->privilegiosFile($estructura['Agenda'],'Block de Notas','listar_nota.php','nuevo_nota.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idnota="+$$("idDelete").value+"&tabla=nota&menu=7";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Nota?';
...
<form name="listado" id="listado" method='get' action='listar_nota.php#t7' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=n.idnota>Nº</option>";
				echo "<option value=n.titulo>Titulo</option>";  
			  ?>
            </select>
   </div> 
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		$cFecha = ($_GET['fecha'] == "") ? "" : " n.fecha ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";
		$condicion = " where n.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and $cFecha ".$_GET['campo']
		." like '".$_GET['abuscar']."%' and n.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= " where n.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and n.estado = ".$_GET['estado'];
		} else { 
			$condicion= ' where n.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and  n.estado =1 ';
		}
	}

	$sql = "SELECT idnota,date_format(fecha,'%d/%m/%Y'),contenido,titulo,left(concat(t.nombre,' ',t.apellido),25)"
	    ." from nota n,trabajador t,usuario u ".$condicion." order by idnota desc";
...
<?php
$nombres = array(  "Id", "Fecha", "Contenido", "Titulo", "Responsable" );
...
$id = $fila[0];
  ...
  if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_nota.php?idnota=".$id."&sw=1#t7' > <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
