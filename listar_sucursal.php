...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Administracion'],'Sucursal','listar_sucursal.php','nuevo_sucursal.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idsucursal="+$$("idDelete").value+"&tabla=sucursal&menu=10";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Sucursal?';
...
<div class="menuTituloFormulario"> Administración > Gestionar Sucursal </div>
...
<form name="listado" id="listado" method='get' action='listar_sucursal.php#t10' enctype='multipart/form-data'>
<table width='79%' border='0' align="center" class="contenedorPrincipal" cellpadding="0" cellspacing="0"> <tr><td bgcolor='#FFF'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
			   echo "<option value=idsucursal>Nº</option>";
	           echo "<option value=direccion>Direccion</option>";
			   echo "<option value=nombrecomercial>Nombre Comercial</option>";
...
<table width='100%' border='0' id='tabla' style='margin-top:20px;'>
  <tr >
...
<?php
	...
	$sql = "SELECT idsucursal,left(nombrecomercial,25)as nombrecomercial ,left(direccion,20)as 'direccion'
	,telefono,left(ciudad,20)as ciudad from sucursal".$condicion." order by idsucursal desc";
...
<?php if ($numeroPagina > 0) { ?>
  <a href="<?php printf("%s?numPage=%d%s#t10", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>

</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t10", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
 <?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t10", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
        <a href="<?php printf("%s?numPage=%d%s#t10", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>">
...
<div style="overflow:auto;position:relative;height:580px;">

<?php

$nombres = array(  "Nº", "Nombre Comercial", "Dirección",  "Teléfono", "Ciudad" );
...
$id = $fila[0];
 if (isset($_GET['estado']) && ($_GET['estado'] == 0)){
	...
	if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_sucursal.php?idsucursal=".$id."&sw=1#t10' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
