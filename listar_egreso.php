...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Contabilidad'],'Egreso de Dinero','listar_egreso.php','nuevo_egreso.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idegreso="+$$("idDelete").value
  +"&tabla=egreso&menu=2&libro=Egreso dinero&tipolibro=egreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Egreso';
...
function accionPostRegistro(){
   window.open('egresos/imprimir_egreso.php?idegreso='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_egreso.php#t2' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idegreso>Nº</option>";
				echo "<option value=recibo>Nº Recibo</option>";
...
$sql = "SELECT idegreso,fecha,left(s.nombrecomercial,25),recibo,cheque,left(nombrepersona,11),round(egresoBolivianos,2)
,round(egresoDolares,2),left(concat(t.nombre,' ',t.apellido),25) ,i.tipocambio from egreso i,
usuario u,trabajador t,sucursal s ".$condicion." order by idegreso desc";
...
<?php if ($numeroPagina > 0) { ?>
   <a href="<?php printf("%s?numPage=%d%s#t2", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t2", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
<?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t2", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t2", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>">
...
if ($fileAcceso['Modificar'] == "Si"){ 
   echo "<td><div align='center'><a href='nuevo_egreso.php?idegreso=".$id."&sw=1#t2' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
} else {
 ...
 echo "<td><div align='center' class='cursorDelete'><img src='css/images/imprimir.gif' title='Imprimir'
  alt='imprimir' border='0' onclick='openVentanaImprimir($id)'/><br /></div></td>";
		...
		else
		{
          switch($i){
			...
			break;
			case 6:
			  echo "<td class='centro'>".number_format($fila[$i],2)."</td>";
			break;
			case 7:
			  echo "<td class='centro'>".number_format(round(($fila[$i]/$fila[9]),2),2)."</td>";
			break;
			default:
