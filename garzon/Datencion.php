<?php
    ...
    if ($tipo == "mesas") {	 
	    $sql = "select max(a.nromesa) as 'nro',turno from atencion a,usuariorestaurante u where 
		a.idusuariorestaurante='$_SESSION[idusuariorestaurante]' and a.idusuariorestaurante=u.idusuario "
			 ." and date(a.fecha)=CURRENT_DATE() group by date(a.fecha)";
		...
		if ($dato['turno'] == "PM") {
			...
			$turno = $db->arrayConsulta($sql);
			$fechaCierre = explode(":", $turno['horacierre']);
			$fechaCierre = ((int) $fechaCierre[0]) + 2;
			if ($hora >= 0 && $hora <= $fechaCierre) {
				$dia = time()- (1*24*60*60); 
                $fechaS = date("Y-m-d H:i:s", $dia );
				$sql = "select max(a.nromesa) as 'nro',turno from atencion a,usuariorestaurante u where 
		        a.idusuariorestaurante='$_SESSION[idusuariorestaurante]' and a.idusuariorestaurante=u.idusuario "
			    ." and date(a.fecha)=DATE_SUB(CURRENT_DATE(),INTERVAL 1 DAY) group by date(a.fecha)";
				$dato = $db->arrayConsulta($sql);
				if ($dato['nro'] == "") {
					 $cantidad = 1;
				} else {
		...
		$sql = "insert into atencion(idatencion,fecha,estado,nromesa,tipo,idtrabajador,idusuariorestaurante"
			 ...
			 . "values(null,'".$fechaS."','atencion',$cantidad,'',null,$_SESSION[idusuariorestaurante]"
