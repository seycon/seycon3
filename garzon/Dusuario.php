<?php
    ...
    if ($tipo == "listaUsuarios") {
	    $i = 0;	
		$sql = "select DAYOFWEEK(now())as 'diasemana';";
		$dia = $db->arrayConsulta($sql);
	    ...
	    for($k = 1; $k <= 2; $k++) {
		 ...
		 while ($dato = mysql_fetch_array($trabajadores)) {
			 ...
			 $vector = explode("-", $dato['diassemana']);
			 $pos = $dia['diasemana'] - 1; 
			 if (($dato['tipo'] == "A" && $vector[$pos] == "1") || $dato['tipo'] == "F" || $dato['total'] > 0) {			 
