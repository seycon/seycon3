<?php
   ...
   if ($tipo == "actualizarAtencion") {
	   ...
	   if (isset($_GET['fecha'])) {
 		 $fechaS = time();
         $hora = date("H:i:s", $fechaS );   
	     $fecha = $db->GetFormatofecha($_GET['fecha'],"/");
	     $auxiliar = ",fecha='$fecha $hora'";
	   }
	   
	   $efectivo = ($_GET['cortesia'] > 0) ? 0 : $_GET['efectivo'];
	   
	   $sql = "update atencion set estado='cobrado',idtrabajador='$_GET[trabajador]',credito=$_GET[credito],"
	       ."socio=$_GET[socio],efectivo='$efectivo' $auxiliar,cortesia='$_GET[cortesia]',descuento='$_GET[descuento]' "
