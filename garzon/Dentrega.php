<?php
    ...
    if ($tipo == "insertar") {	
		...
		$sql = "select u.idsucursal,s.nombrecomercial from  usuariorestaurante u,sucursal s where u.idsucursal=s.idsucursal
		 and u.tipo='$_GET[tipo]' and u.idtrabajador=$_GET[idtrabajador] and u.estado=1;";
	    ...
	    $sql = "insert into entregadinero(identrega,fecha,monto,acumulado,idtrabajador,tipo,idusuariorestaurante,estado,caja,
		  nulo,cortesia,credito,idusuariosistema,idsucursal) value(null,'$fecha','$_GET[monto]'
		  ,'$_GET[acumulado]','$_GET[idtrabajador]'
		  ,'$_GET[tipo]','$_SESSION[id_usuario]',1,'$_GET[caja]','$_GET[nulo]','$_GET[cortesia]','$_GET[credito]',
		  '$_SESSION[id_usuario]','$sucursal[idsucursal]');";
	    $db->consulta($sql);
	...
	if ($tipo == "modificar") {
	    ...
	    $sql = "update entregadinero set fecha='$fecha',idsucursal='$sucursal[idsucursal]'
		    ,monto='$_GET[monto]',acumulado='$_GET[acumulado]'
