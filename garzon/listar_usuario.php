<?php
    ...
    if (isset($_GET['idusuario']) && $_GET['idusuario'] != "") {
        $sql = "update usuariorestaurante set estado=0 where idusuario=$_GET[idusuario]";	 
        $db->consulta($sql);
...
<script>
	...
	var goEliminar = function(codigo){
	  ...
	  var filtro = "tipo=bajaUsuario&usuario="+ codigo;
	  enviar("Dusuario.php",filtro,resultadoEliminacion);
	}
	...
	var resultadoEliminacion = function(resultado){
	  ...
	  if (parseFloat(resultado) > 0) {
		$$("modal_contenido").innerHTML = 'Antes de eliminar el usuario debe cobrar sus mesas pendientes.';  
 ...
 <div class="contendedor">
  ...
  <tr>
    ...
    <td width="79%">
      <div class="menu2">
       ...
       <form id="formulario" name="formulario" method="get" action="listar_usuario.php">
       <?php
	    ...
	    $sql = "(select u.idusuario,u.tipo,left(concat(t.nombre,' ',t.apellido),20)as 'nombre'"
			 ...
			 ." u.tipo='fijo' and u.estado=1 and s.idsucursal=u.idsucursal $condicion)union all"
			 ...
			 ." u.tipo='apoyo' and u.estado=1 and s.idsucursal=u.idsucursal $condicion) order by idusuario desc";
  ...
  <table width="100%" border="0">
    ...
    <td width="11%"><select id="campo" name="campo" class="combo">
      <?php
		 $selec = $_GET['campo']; 
		 $tipo = array("u.idusuario", "t.nombre", "s.nombrecomercial");
    ...
    <td width="6%"></td>
    <td width="4%"><input type="hidden" id="idusuario" name="idusuario"  value=""/></td>
  </tr>
