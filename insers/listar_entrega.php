<?php
    ...
    if (isset($_POST['identrega']) && $_POST['identrega'] != "") {
        $sql = "update entregadinero set estado=0 where identrega=$_POST[identrega]";	 
        $db->consulta($sql);		
		
		$sql = "select idlibrodiario,idsucursal from librodiario where transaccion='Entrega Dinero Restaurante'"
	    ." and idtransaccion=$_POST[identrega];"; 
...
<div id="modal_mensajes" class="modal_mensajes">
 ...
 <div class="contendedor">
  ...
  <tr>
    ...
    <td width="79%">
      <div class="menu2">
       <div class="tituloTransaccion"><div class="textoTituloTransaccion">Listar Entrega</div></div>
          <div class="separador"></div>
       ...
       <form id="formulario" name="formulario" method="post" action="listar_entrega.php">
       <?php
	    ...
	    $sql = "(select u.identrega,u.fecha,u.monto,left(concat(t.nombre,' ',t.apellido),20)as 'nombre'"
		   ...
		   ." u.tipo='fijo' and u.estado=1 and s.idsucursal=a.idsucursal and a.idtrabajador=u.idtrabajador "
		   ...
		   ." u.tipo='apoyo' and u.estado=1 and s.idsucursal=a.idsucursal $condicion) order by identrega desc";
  ...
  <tr>
    ...
    <td width="11%"><input type="text"  name="filtro" id="filtro" class="caja_busqueda"/></td>
    <td width="11%"><select id="campo" name="campo" class="combo">
      <option value="u.identrega">Nº</option>
      <option value="t.nombre">Nombre</option>
