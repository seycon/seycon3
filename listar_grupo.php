...
<?php
 ...
 $fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Grupo y Subgrupo','listar_grupo.php','nuevo_grupo.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idgrupo="+$$("idDelete").value+"&tabla=grupo&menu=9";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Grupo?';
...
<form name="listado" id="listado" method='get' action='listar_grupo.php#t9' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=idgrupo>Nº</option>";
				echo "<option value=nombre>Nombre Grupo</option>";		  
			  ?>
            </select>
   </div> 
...
<?php
	...
	$sql = 'SELECT idgrupo,nombre from grupo '.$condicion.' order by idgrupo desc';
...
<?php if ($numeroPagina > 0) { ?>
  <a href="<?php printf("%s?numPage=%d%s#t9", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>

</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t9", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
 <?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t9", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
        <a href="<?php printf("%s?numPage=%d%s#t9", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
$nombres = array(  "Nº", "Nombre Grupo"  );
echo "<table border='0'  align='center'>";
echo "<table width='100%' border='0' align='center'>";
echo "<thead><tr class='cabeceraInicialListar'>";
echo "<th width='30px'>&nbsp;</th><th width='30px'>&nbsp;</th>";
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_grupo.php?idgrupo=".$id."&sw=1#t9' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
