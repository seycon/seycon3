...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Control Inventario'
	,'listar_inventario.php','nuevo_inventario.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idinventario="+$$("idDelete").value+"&tabla=inventario&menu=3";
}
...
var openMensaje = function(codigo){
    ...
    $$("modal_contenido").innerHTML = '¿Desea anular este Inventario.?';
	$$("modal_boton2").style.visibility = "visible"; 
...
var closeMensaje = function(){
	...
	$$("modal_boton2").style.visibility = "hidden"; 
}
...
function accionPostRegistro(){
   window.open('inventario/imprimir_inventario.php?idinventario='+$$("idDelete").value+'&logo='+$$("logo").checked
   +'&grupo='+$$("grupoR").value,'target:_blank');	
   cerrarPagina();
}
 ...
 <div id="modal_mensajes" class="contenedorMsgBox">
  ...
  <div class="modalContenidoMsgBox">
      ...
      <div class="contenidoMsgBox">
        ...
        <div id="modal_boton2" class="modal_boton2MsgBox">
  ...
  <div class="modalContenidoMsgBoxOption">
      ...
      <div class="contenidoMsgBoxOption">
        ...
        poder visualizar el reporte.  </div>
         <table width="395" align="center" style="margin-top:40px;">
            <tr>            
            <td width="91" align="center">
            <input type="button" value="Ver Reporte" onclick="accionPostRegistro();" class="botonseycon"/></td>
            <td width="48" align="right">Grupo:</td>
            <td width="84">
            ...
            <td width="128" align="right">Logo:</td>
...
<form name="listado" id="listado" method='get' action='listar_inventario.php#t3' enctype='multipart/form-data'>
<table width='76%' border='0' class="contenedorPrincipal"> <tr><td bgcolor='#FFF'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				echo "<option value=p.idinventario>Nº</option>";
				echo "<option value=a.nombre>Almacén</option>";
				echo "<option value=p.administrador>Administrador</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		$cFecha = ($_GET['fecha'] == "") ? "" : " p.fechainicio ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";	
		$condicion= "   $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and p.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion= "   p.estado = ".$_GET['estado'];
		} else { 
			$condicion= '  p.estado =1';
		}
	}

	$sql = "SELECT p.idinventario, p.fechainicio, p.fechafinal, left(a.nombre,15)as almacen, p.administrador,
        left(concat(t.nombre,' ',t.apellido),25)
        from inventario p,almacen a,usuario u,trabajador t where u.idusuario=p.idusuario 
		and u.idtrabajador=t.idtrabajador  and p.idalmacen=a.idalmacen and ".$condicion." order by idinventario desc";
...
<?php if ($numeroPagina > 0) { ?>
  <a href="<?php printf("%s?numPage=%d%s#", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>

</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
 <?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
        <a href="<?php printf("%s?numPage=%d%s#", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
$nombres = array(  "idinventario",  "Fecha Inicio", "Fecha Final",  "Almacén", "Administrador", "Usuario" );
...
if ($fileAcceso['Modificar'] == "Si"){ 
	echo "<td><div align='center'><a href='nuevo_inventario.php?idinventario=".$id."&sw=1#t3' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
