...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Activo'],'Alta de Activo','listar_activo.php','nuevo_activo.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idactivo="+$$("idDelete").value+"&tabla=activo&menu=1&libro=Nuevo Activo&tipolibro=ingreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Activo?';
...
function accionPostRegistro(){
   window.open('activo/imprimir_altaactivo.php?idactivo='+$$("idDelete").value,'target:_blank');	
   cerrarPagina();
}
  ...
  <div class="modalContenidoMsgBoxOption">
      ...
      <div class="contenidoMsgBoxOption">
        ...
        <table width="311" style="margin-top:40px;" align="center">
          <tr>            
           <td align="center"><input type="button" value="Ver Reporte" onclick="accionPostRegistro();" class="botonNegro"/></td>
          </tr>
        </table>   
...
<form name="listado" id="listado" method='get' action='listar_activo.php#t1' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo1">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				  echo "<option value=a.numero>Nº</option>";
				  echo "<option value=a.nombre>Nombre</option>";
				  echo "<option value=a.ubicacion>Ubicacion</option>";
				  echo "<option value=s.nombrecomercial>Sucursal</option>";
				  echo "<option value=r.nombre>Responsable</option>";			  
			  ?>
            </select>
   </div> 
...
<?php
	...
	if ($_GET['abuscar'] != '') {
		$condicion = " and ".$_GET['campo']." like '".$_GET['abuscar']."%' and a.estado=".$_GET['estado'];
	} else {
		if (isset($_GET['estado'])) {
			$condicion = "  and a.estado = ".$_GET['estado'];
		} else { 
			$condicion = "  and a.estado = 1";
		}
	}

	$sql = "SELECT idactivo,a.numero,left(a.nombre,25),a.ubicacion, t.nombre,left(s.nombrecomercial,25)as"
	    ." 'nombrecomercial',left(concat(r.nombre,' ',r.apellido),25)  from activo a,"
	    ."sucursal s,tipoactivo t,trabajador r,usuario u  where a.idtipoactivo=t.idtipoactivo "
	    ."and s.idsucursal=a.idsucursal and a.idusuario=u.idusuario "
	    ."and u.idtrabajador=r.idtrabajador ".$condicion." order by idactivo desc";
...
<?php
$nombres = array(  "Nº", "Nombre", "Ubicación","Tipo Activo","Sucursal","Responsable");
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
 ...
 if ($fileAcceso['Modificar'] == "Si") { 
	echo "<td><div align='center'><a href='nuevo_activo.php?idactivo=".$id."&sw=1#t1' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
 } else {
