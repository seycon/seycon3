...
<?php 
	...
	$db = new MYSQL();
	$idcontac = $db->getMaxCampo('iddepartamento', 'departamento');
	if (!isset($_SESSION['softLogeoadmin'])) {
		header("Location: index.php");	
	}
	$estructura = $_SESSION['estructura'];
	$fileAcceso = $db->privilegiosFile($estructura['Recursos'],'Departamento','nuevo_departamento.php','listar_departamento.php');
	...
	if (isset($_POST['transaccion'])) {	
	   if ($_POST['transaccion'] == "insertar") {
		 ...
		 (iddepartamento,nombre,descripcion,estado,idusuario)
		  VALUES (NULL,'".filtro($_POST['nombre'])."','".filtro($_POST['descripcion'])."','1',
		 $_SESSION[id_usuario]);";    
	   }
	 
	  if ($_POST['transaccion'] == "modificar") {
		  $sql = "UPDATE departamento SET nombre='".filtro($_POST['nombre'])
		  ."', descripcion='".filtro($_POST['descripcion'])."', 
		  idusuario=$_SESSION[id_usuario] WHERE iddepartamento= '".$_POST['iddepartamento']."';";	 
	  }
	
	  $db->consulta($sql);
	  header("Location: nuevo_departamento.php#t4");
	}
	
	$transaccion = "insertar";
	if (isset($_GET['sw'])) {
	    $transaccion = "modificar";	
	    $sql = "SELECT * FROM departamento WHERE iddepartamento = ".$_GET['iddepartamento'];
	    $datoDepartamento = $db->arrayConsulta($sql);  
	}
...
<script>
  ...
  document.onkeydown = function(e){
   ...
   if (tecla == 115){ //F4
    if ($$("cancelar") != null) 
	 location.href = 'listar_departamento.php#t4';
   }
...
<div class="contemHeaderTop">
    ...
    <?php 
        if ($fileAcceso['File'] == "Si") {
		 ...
		 value="Cancelar [F4]" onClick="location.href=&#039listar_departamento.php#t4&#039"/>';	
        }
    ?>
     
     </td>
    <td><input type="hidden"  id="transaccion" name="transaccion" value="<?php echo $transaccion;?>" />
    <input type="hidden" id="iddepartamento" name="iddepartamento" value="<?php echo $datoDepartamento['iddepartamento'];?>" /></td>
    ...
    <table width="356" border="0">
      <tr>
        ...
        if(isset($_GET['iddepartamento'])) {
            echo $_GET['iddepartamento'];
        } else {
            echo $db->getNextID('iddepartamento', 'departamento');
        }
        ?></td>
      </tr>
      <tr>
...
<tr>
  ...
  <table width="90%" border="0" align="center" class="bordeContenido">
    ...
    <tr>
      ...
      <td>
      <input type='text' id="nombre" name="nombre"  class="required" size="25" value="<?php echo $datoDepartamento['nombre'];?>"/>
      </td>
      <td align="right"></td>
      <td></td>
      </tr>
    ...
    <tr>
      ...
      <textarea name="descripcion" id="descripcion" cols="80" rows="3"><?php echo $datoDepartamento['descripcion'];?></textarea>
      </td>
      </tr>
    <tr>
      <td>&nbsp;</td>
