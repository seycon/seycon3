...
<?php
	...
	,'listar_traspasoDinero.php','nuevo_traspaso_dinero.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idtraspaso="+$$("idDelete").value+"&tabla=traspaso&reg=traspasodinero&menu=1&libro=Traspaso dinero&tipolibro=traspaso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Traspaso?';
...
function accionPostRegistro(){
   window.open('traspaso/imprimir_traspaso.php?idtraspaso='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_traspasoDinero.php#t8' enctype='multipart/form-data'>
...
if (isset($_GET['abuscar'])){
 ...
 $condicion = " where i.idsucursal=s.idsucursal and i.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and ".$_GET['campo']." like '".$_GET['abuscar']."%' and $cFecha i.estado=".$_GET['estado'];
}
else 
if (isset($_GET['estado'])) 
$condicion = " where i.idsucursal=s.idsucursal and i.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and i.estado = ".$_GET['estado'];
else  
$condicion = ' where i.idsucursal=s.idsucursal and i.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and i.estado = 1';

$sql = "SELECT idtraspaso,fecha,left(s.nombrecomercial,25),recibo,cheque,round(ingresoBolivianos,2),round(ingresoDolares,2),left(concat(t.nombre,' ',t.apellido),25),i.tipocambio from traspasodinero i,usuario u,trabajador t,sucursal s ".$condicion." order by idtraspaso desc";
...
<?php if ($numeroPagina > 0) { ?>
   <a href="<?php printf("%s?numPage=%d%s#t8", $currentPage, 0, $consultaRegistro); ?>"><img src="images/first.png" title="Primero"/></a>
<?php } ?>
</td>
<td width='17'>
<?php if ($numeroPagina > 0) {  ?>  
    <a href="<?php printf("%s?numPage=%d%s#t8", $currentPage, max(0, $numeroPagina - 1), $consultaRegistro); ?>">
    <img src="images/prev.png" title="Anterior"/></a>
<?php }?>
</td>
<td width='17'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
<a href="<?php printf("%s?numPage=%d%s#t8", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t8", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
<?php
	$nombres = array(  "Nº", "fecha","Sucursal", "Doc.", "Cheque","Bs.", "us.", "Usuario" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
    ...
    if ($fileAcceso['Modificar'] == "Si") { 
	 echo "<td><div align='center'><a href='nuevo_traspaso_dinero.php?idtraspaso=".$id."&sw=1#t8'
	...
	for( $i = 0; $i <= 7; $i++ ) {
		...
		else
		{
          switch($i){
			...
			break;
			case 6:
			  echo "<td class='centro'>".number_format(round(($fila[$i]/$fila[8]),2),2)."</td>";
			break;
			default:
