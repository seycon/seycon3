...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Ruta de Entrega','listar_rutaentrega.php','nuevo_rutaentrega.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idrutaentrega="+$$("idDelete").value+"&tabla=rutaentrega&menu=1";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Ruta de Entrega?';
...
function accionPostRegistro(){
   window.open('rutaentrega/imprimir_rutaentrega.php?identrega='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<form name="listado" id="listado" method='get' action='listar_rutaentrega.php' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
					echo "<option value=r.idrutaentrega>Nº</option>";
...
<?php
	...
	if (isset($_GET['abuscar'])) {
	  ...
	  $condicion = " and $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and r.estado=".$_GET['estado'];
	} else {
	  if (isset($_GET['estado'])) {
		  $condicion = " and r.estado = ".$_GET['estado'];
	  } else {
		  $condicion = " and r.estado = 1";
	  }
	}
	
	
	$sql = "select r.idrutaentrega,r.fecha,left(concat(t.nombre,' ',t.apellido),20)as 'nombre' 
	from rutaentrega r,trabajador t,usuario u where r.idusuario=u.idusuario and 
	u.idtrabajador=t.idtrabajador  ".$condicion." order by r.idrutaentrega desc";
...
$nombres = array(  "Nº", "Fecha", "Usuario" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)){	
    ...
    if ($fileAcceso['Modificar'] == "Si") { 
	   ...
	   <a href='nuevo_rutaentrega.php?idrutaentrega=".$id."&sw=1#t1' onclick='return modificar(this)'>
	   <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	} else {
	   echo "<td>
	   <div align='center'><img src='css/images/edit.gif' title='Sin Privilegios para Modificar' alt='editar' border='0' />
	   </div></td>";	
	}
