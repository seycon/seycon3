...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Inventario'],'Egreso de Productos'
	,'listar_egresoproducto.php','nuevo_egresoalmacen.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idegresoprod="+$$("idDelete").value
  +"&tabla=egresoprod&menu=4&reg=egresoproducto&libro=Egreso Almacen&tipolibro=egreso";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular este Egreso de Almacen?';
...
function accionPostRegistro(){
   window.open('egresoalmacen/imprimir_egresoProductos.php?idegreso='+$$("idDelete").value+'&logo='+$$("logo").checked
...
<form name="listado" id="listado" method='get' action='listar_egresoproducto.php#t4' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
				  	echo "<option value=idegresoprod>Nº</option>";
					echo "<option value=a.nombre>Almacén</option>";
					echo "<option value=e.monto>Monto</option>";			  
...
if (isset($_GET['abuscar'])){
 $cFecha = ($_GET['fecha'] == "") ? "" : " e.fecha ='".$db->GetFormatofecha($_GET['fecha'],'/')."' and ";	
 $condicion = " and e.idalmacen = a.idalmacen and $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and e.estado=".$_GET['estado'];
}
else 
if (isset($_GET['estado'])) 
$condicion= " and e.idalmacen = a.idalmacen and e.estado = ".$_GET['estado'];
else 
$condicion= '  and e.idalmacen = a.idalmacen and e.estado =1';


$sql = "select idegresoprod,e.fecha,left(a.nombre,20)as nombre,left(
IF( e.cuentacontable !=  '', (
SELECT pc.cuenta
FROM plandecuenta pc
WHERE pc.codigo = e.cuentacontable
AND pc.estado =1
),0)
,20)as motivo,round(e.monto,2)as monto
,left(concat(t.nombre,' ',t.apellido),30)as responsable  
from egresoproducto e,trabajador t,usuario u,almacen a where 
u.idusuario=e.idusuario and u.idtrabajador=t.idtrabajador ".$condicion." order by idegresoprod desc";
...
$nombres = array(  "idegresoprod", "Fecha", "Almacén", "Cuenta Contable", "monto" ,"Usuario");
...
if ($fileAcceso['Modificar'] == "Si"){ 
	 echo "<td><div align='center'><a href='nuevo_egresoalmacen.php?idegresoprod=".$id."&sw=1#t4' onclick='return modificar(this)'> <img src='css/images/edit.gif' title='Modificar' alt='editar' border='0' /><br /></a></div></td>";
	}else{
