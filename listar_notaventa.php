...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Venta de Producto','listar_notaventa.php','nuevo_notaventa.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idnotaventa="+$$("idDelete").value+"&tabla=notaventa&menu=2&libro=Nota Venta Productos&tipolibro=ingreso";
}
...
function accionPostRegistro(){
   window.open('factura/imprimir_notaventa.php?idnotaventa='+$$("idDelete").value+'&logo='+$$("logo").checked,'target:_blank');	
   cerrarPagina();
}
...
<div class="cabeceraFormulario">
  <div class="menuTituloFormulario"> Ventas > Venta de Producto </div>
...
<form name="listado" id="listado" method='get' action='listar_notaventa.php#t2' enctype='multipart/form-data'>
...
<?php
	...
	if (isset($_GET['abuscar'])) {
		...
		$condicion = " where n.idcliente = c.idcliente and n.idsucursal=s.idsucursal and 
		 ...
		 and  $cFecha  n.tiponota='productos' and "
		.$_GET['campo']." like '".$_GET['abuscar']."%' and n.estado=".$_GET['estado'];
	} else  {
		if (isset($_GET['estado'])) {
			$condicion = " where n.idcliente = c.idcliente and n.idsucursal=s.idsucursal and 
			n.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador 
			and n.tiponota='productos' and n.estado = ".$_GET['estado'];
		} else {
			$condicion = " where n.idcliente = c.idcliente and n.idsucursal=s.idsucursal and 
			n.idusuario=u.idusuario and u.idtrabajador=t.idtrabajador and n.tiponota='productos' and n.estado = 1";
		}
	}	
	
	$sql = "SELECT idnotaventa,left(concat(c.nombre),25),numfactura,fecha,round(monto,2),left(s.nombrecomercial,25)
	,n.moneda,n.tipocambio,n.numero,left(concat(t.nombre,' ',t.apellido),20) from notaventa n,cliente c,usuario u,trabajador t
	,sucursal s ".$condicion." order by idnotaventa desc";
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)){	
	...
	if ($fileAcceso['Modificar'] == "Si") { 
		 echo "<td><div align='center'><a href='nuevo_notaventa.php?idnotaventa=".$id."&sw=1#t2'
