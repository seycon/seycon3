...
<?php
	...
	$fileAcceso = $db->privilegiosFile($estructura['Ventas'],'Cotización','listar_cotizacion.php','nuevo_cotizacion.php');
...
function eliminarTransaccion(){
  location.href = "anular_registro.php?idcotizacion="+$$("idDelete").value+"&tabla=cotizacion&menu=1";
}
...
var openMensaje = function(codigo){
	...
	$$("modal_contenido").innerHTML = '¿Desea anular esta Cotizacion?';
...
function accionPostRegistro(){
   window.open('cotizaciones/imprimir_cotizacion.php?idcotizacion='+$$("idDelete").value
...
<form name="listado" id="listado" method='get' action='listar_cotizacion.php#t1' enctype='multipart/form-data'>
      ...
      <div class="listadoCombo2">
          <div class="borde1Listar"> </div>
            <select id="campo" name="campo" class="borde2Combo1">
              <?php
					echo "<option value=idcotizacion>Nº</option>";
                 	echo "<option value=c.moneda>Moneda</option>";
                	echo "<option value=formapago>Forma de Pago</option>";
                	echo "<option value=t.nombrenit>Cliente</option>";
...
if (isset($_GET['abuscar'])) {
  ...
  $condicion = " where c.idcliente=t.idcliente 
    and c.idusuario=u.idusuario and u.idtrabajador=tb.idtrabajador 
    and $cFecha ".$_GET['campo']." like '".$_GET['abuscar']."%' and c.estado=".$_GET['estado'];
} else {
  if (isset($_GET['estado'])) {
	  $condicion = " where c.idcliente=t.idcliente and c.idusuario=u.idusuario 
	  and u.idtrabajador=tb.idtrabajador and c.estado = ".$_GET['estado'];
  } else {
	  $condicion = " where c.idcliente=t.idcliente and c.idusuario=u.idusuario 
	  and u.idtrabajador=tb.idtrabajador and c.estado = 1";
  }
}


$sql = "SELECT idcotizacion,fecha,left(t.nombrenit,25),c.moneda,formapago,round(monto,2)
,left(concat(tb.nombre,' ',tb.apellido),20) from cotizacion c,cliente t
,usuario u,trabajador tb "
.$condicion." order by idcotizacion desc";
...
<a href="<?php printf("%s?numPage=%d%s#t1", $currentPage, min($totalPages_Recordset1, $numeroPagina + 1), $consultaRegistro); ?>"><img src="images/next.png" title="Siguiente"/></a>
 <?php } ?>
</td>
<td width='21'>
<?php if ($numeroPagina < $totalPages_Recordset1) {  ?>
  <a href="<?php printf("%s?numPage=%d%s#t1", $currentPage, $totalPages_Recordset1, $consultaRegistro); ?>"><img src="images/last.png" 
        title="Ultimo"/></a>
<?php }  ?>
</td>
<td width='32'>&nbsp;</td>
...
$nombres = array(  "Nº", "Fecha", "Cliente", "Moneda", "Forma de Pago", "Total", "Usuario" );
...
if (isset($_GET['estado']) && ($_GET['estado'] == 0)) {	
	...
	echo "<td><div align='center'><a href='nuevo_cotizacion.php?idcotizacion=".$id."&sw=1#t1'>
